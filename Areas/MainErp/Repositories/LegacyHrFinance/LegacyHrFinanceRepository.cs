        ...
        public LegacyHrFinancePageViewModel Load(string moduleKey, string searchText, int page, int pageSize)
        {
            ...
            using (var connection = _connectionFactory.CreateOpenConnection())
            {
                switch ((moduleKey ?? string.Empty).ToLowerInvariant())
                {
                    ...
                    case "advances": return LoadAdvances(connection, searchText, page, pageSize);
                    case "leave": return LoadLeaveEntitlements(connection, searchText, page, pageSize);
                    case "sickleave": return LoadSickLeaves(connection, searchText, page, pageSize);
                    case "adjustments": return LoadAdjustments(connection, searchText, page, pageSize);
...
WHERE id = @Id;", connection, transaction))
        ...
        {
            var model = Base("components", "مكونات الرواتب", "Main Original VB6", "MOFRAD", "mofrad", null, searchText, page, pageSize);
...
SELECT * FROM (
  ...
  WHERE @Search = N'' OR ISNULL(name, N'') LIKE N'%' + @Search + N'%' OR ISNULL(nameE, N'') LIKE N'%' + @Search + N'%' OR ISNULL(Account_Code, N'') LIKE N'%' + @Search + N'%'
        ...
        private LegacyHrFinancePageViewModel LoadAdvances(SqlConnection connection, string searchText, int page, int pageSize)
        {
            var model = Base("advances", "طلبات السلف", "الموارد البشرية", "طلبات السلف", "TblEmpAdvanceRequest", "الحفظ والترحيل المحاسبي لهذه الشاشة محميان حتى تثبيت دورة الاعتماد بالكامل.", searchText, page, pageSize);
            using (var command = new SqlCommand(@"
SELECT * FROM (
 ...
 WHERE @Search = N'' OR ISNULL(e.Emp_Name,N'') LIKE N'%' + @Search + N'%' OR CONVERT(NVARCHAR(30), a.AdvanceID) = @Search
) q WHERE RowNo BETWEEN @Start AND @End ORDER BY RowNo;", connection))
            {
                AddSearch(command, searchText, page, pageSize);
        ...
        private LegacyHrFinancePageViewModel LoadLeaveEntitlements(SqlConnection connection, string searchText, int page, int pageSize)
        {
            var model = Base("leave", "إدارة مستحقات الإجازات", "الموارد البشرية", "مستحقات الإجازات", "TblVocationEntitlements", "شاشة قراءة ومراجعة فقط لحين تثبيت معادلات الاستحقاق النهائية.", searchText, page, pageSize);
            using (var command = new SqlCommand(@"
SELECT * FROM (
 ...
 WHERE @Search = N'' OR ISNULL(e.Emp_Name,N'') LIKE N'%' + @Search + N'%' OR CONVERT(NVARCHAR(30), v.ID) = @Search
) q WHERE RowNo BETWEEN @Start AND @End ORDER BY RowNo;", connection))
            {
                AddSearch(command, searchText, page, pageSize);
        ...
        private LegacyHrFinancePageViewModel LoadSickLeaves(SqlConnection connection, string searchText, int page, int pageSize)
        {
            var model = Base("sickleave", "الإجازات المرضية", "الموارد البشرية", "الإجازات المرضية", "TblRegsterSickleave", "التسجيل الجديد يحتاج تحقق التداخل وتأثير الرواتب؛ لذلك الشاشة محمية الآن.", searchText, page, pageSize);
            using (var command = new SqlCommand(@"
SELECT * FROM (
 ...
 WHERE @Search = N'' OR ISNULL(e.Emp_Name,N'') LIKE N'%' + @Search + N'%' OR CONVERT(NVARCHAR(30), s.ID) = @Search
) q WHERE RowNo BETWEEN @Start AND @End ORDER BY RowNo;", connection))
            {
                AddSearch(command, searchText, page, pageSize);
        ...
        private LegacyHrFinancePageViewModel LoadAdjustments(SqlConnection connection, string searchText, int page, int pageSize)
        {
            var model = Base("adjustments", "تعديلات مكونات الرواتب", "الموارد البشرية", "تعديلات مكونات الرواتب", "TblChangedComponentRegister / Details", "الترحيل والحذف المحاسبي غير مفعلين حتى تثبيت كل قواعد الاعتماد؛ العرض والبحث جاهزان.", searchText, page, pageSize);
            using (var command = new SqlCommand(@"
SELECT * FROM (
...
SELECT * FROM (
 ...
 GROUP BY a.EmpAllocationsid, a.RecordDate, a.[year], a.[month], a.AllocationsType, a.NoteSerial
        ...
        private static LegacyHrFinancePageViewModel Base(string module, string title, string source, string form, string table, string warning, string search, int page, int pageSize)
        {
            return new LegacyHrFinancePageViewModel { ModuleKey = module, Title = title, SourceSystem = source, SourceForm = form, LegacyTable = table, Warning = warning, SearchText = search, Page = page, PageSize = pageSize };
        }
