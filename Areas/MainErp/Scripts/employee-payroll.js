(function () {
    ...
    function storageKey(name) {
        return "MainErp.EmployeePayroll." + name;
    }
    function readStoredRows(name) {
        try { return JSON.parse(localStorage.getItem(storageKey(name)) || "[]"); } catch (ignore) { return []; }
    }
    function writeStoredRows(name, rows) {
        try { localStorage.setItem(storageKey(name), JSON.stringify((rows || []).slice(0, 6))); } catch (ignore) { }
    }
    function rememberRecentEmployee(employee) {
        if (!employee || !employee.EmployeeId) { return; }
        var rows = readStoredRows("RecentEmployees").filter(function (x) { return String(x.EmployeeId) !== String(employee.EmployeeId); });
        rows.unshift({
        ...
        writeStoredRows("RecentEmployees", rows);
        renderRecentEmployees();
    }
    function renderRecentEmployees() {
        var host = byId("epRecentEmployees");
        if (!host) { return; }
        var rows = readStoredRows("RecentEmployees");
        host.hidden = rows.length === 0;
        host.innerHTML = rows.length ? '<strong>آخر ملفات مفتوحة</strong>' : "";
        rows.forEach(function (x) {
            host.insertAdjacentHTML("beforeend", '<button type="button" data-recent-employee="' + html(x.EmployeeId) + '"><span>' + html(x.EmployeeCode || "") + '</span><b>' + html(x.EmployeeName || "Employee") + '</b><small>' + html(x.BranchName || "") + '</small></button>');
        });
    }
    function setRunMonthOffset(offset) {
        var year = parseInt(byId("epRunYear") && byId("epRunYear").value, 10) || new Date().getFullYear();
        var month = parseInt(byId("epRunMonth") && byId("epRunMonth").value, 10) || (new Date().getMonth() + 1);
        var date = new Date(year, month - 1 + offset, 1);
        byId("epRunYear").value = date.getFullYear();
        byId("epRunMonth").value = date.getMonth() + 1;
        message("تم تغيير فترة المسير إلى " + byId("epRunMonth").value + "/" + byId("epRunYear").value);
    }
    function setCurrentRunMonth() {
        var now = new Date();
        byId("epRunYear").value = now.getFullYear();
        byId("epRunMonth").value = now.getMonth() + 1;
        message("تم اختيار الشهر الحالي");
    }
    ...
    function insuranceStateLabel(employee) {
        var mi = employee && employee.MedicalInsurance ? employee.MedicalInsurance : null;
        if (mi && mi.IsActive) { return { text: "مؤمن", cls: "insured" }; }
        if (mi && mi.PlanId && !mi.IsActive) { return { text: "موقوف", cls: "inactive" }; }
        if (employee && employee.HasMedicalInsurance) { return { text: "مؤمن", cls: "insured" }; }
        if (employee && employee.MedicalInsurancePlanName) { return { text: "قيد المراجعة", cls: "pending" }; }
        return { text: "غير مربوط", cls: "excluded" };
    }
    function renderEmployeeDashboard(rows) {
        rows = rows || [];
        var branches = {};
        var insured = 0;
        var pending = 0;
        rows.forEach(function (x) {
            ...
            var state = insuranceStateLabel(x);
            if (state.cls === "insured") { insured++; }
            if (state.cls === "pending" || state.cls === "excluded") { pending++; }
        });
        setText("epDashEmployees", rows.length);
        setText("epDashInsured", insured);
        setText("epDashPendingInsurance", pending);
    ...
    function renderEmployeeProfile(employee) {
        ...
        if (!host) { return; }
        if (!employee) {
            host.innerHTML = '<div class="ep-profile-empty"><strong>اختر موظفاً</strong><span>سيظهر هنا ملخص سريع للفرع، القسم، الراتب، والتأمين الطبي.</span></div>';
            return;
        }
        var state = insuranceStateLabel(employee);
        var mi = employee.MedicalInsurance || {};
        host.innerHTML =
            '<div class="ep-profile-avatar">' + html((employee.EmployeeName || "?").charAt(0)) + '</div>' +
            '<div class="ep-profile-main"><div class="ep-profile-title"><h3>' + html(employee.EmployeeName || "موظف") + '</h3><span class="ep-state-chip ' + state.cls + '">' + state.text + '</span></div>' +
            '<p>' + html(employee.EmployeeCode || "") + ' · ' + html(employee.JobTypeName || "وظيفة غير محددة") + '</p>' +
            '<div class="ep-profile-metrics"><span><b>الفرع</b>' + html(employee.BranchName || "غير محدد") + '</span><span><b>القسم</b>' + html(employee.DepartmentName || "غير محدد") + '</span><span><b>الراتب</b>' + money(employee.BasicSalary) + '</span><span><b>خطة التأمين</b>' + html(mi.PlanName || employee.MedicalInsurancePlanName || "غير مربوطة") + '</span></div></div>';
    }
    function updateInsuranceStateBar(employee) {
        var bar = byId("epInsuranceStateBar");
        if (!bar) { return; }
        var mi = employee && employee.MedicalInsurance ? employee.MedicalInsurance : {};
        var state = insuranceStateLabel(employee || {});
        var payrollLinked = mi.IsMonthly !== false && mi.IsActive;
        bar.innerHTML =
            '<span class="ep-state-chip ' + state.cls + '">' + state.text + '</span>' +
            '<span class="ep-state-chip ' + (payrollLinked ? "payroll" : "inactive") + '">' + (payrollLinked ? "مرتبط بالمسير" : "غير مؤثر حالياً") + '</span>' +
            '<span class="ep-state-chip ' + (mi.EndDate ? "pending" : "insured") + '">' + (mi.EndDate ? "له تاريخ نهاية" : "مفتوح") + '</span>';
    }
    ...
    function loadEmployeesModern() {
        var url = root.getAttribute("data-search-url") + "?" + queryString(employeeFilter());
        var tbody = byId("epEmployeesRows");
        var loadState = byId("epEmployeeLoadState");
        if (loadState) { loadState.textContent = "جار تحميل بيانات الموظفين..."; }
        if (tbody) { tbody.innerHTML = '<tr><td colspan="9" class="ep-empty-row">جار التحميل...</td></tr>'; }
        return getJson(url).then(function (res) {
            var rows = res.rows || [];
            root._employeeRows = rows;
            if (!tbody) { return; }
            tbody.innerHTML = "";
            renderEmployeeDashboard(rows);
            if (!rows.length) {
                tbody.innerHTML = '<tr><td colspan="9" class="ep-empty-row">لا توجد بيانات مطابقة للفلاتر الحالية.</td></tr>';
                renderEmployeeProfile(null);
                if (loadState) { loadState.textContent = "لا توجد نتائج"; }
                return;
            }
            rows.forEach(function (x) {
                var state = insuranceStateLabel(x);
                tbody.insertAdjacentHTML("beforeend",
                    '<tr><td><button type="button" class="ep-link-button" data-profile="' + html(x.EmployeeId) + '">' + html(x.EmployeeCode) + '</button></td><td><strong>' + html(x.EmployeeName) + '</strong><small>' + html(x.Mobile || x.Phone || "") + '</small></td><td>' + html(x.BranchName) + '</td><td>' + html(x.DepartmentName) + '</td><td>' + html(x.JobTypeName) + '</td><td>' + money(x.BasicSalary) + '</td><td><span class="ep-state-chip ' + state.cls + '">' + state.text + '</span></td><td><span class="ep-pill ' + (x.IsActive ? "" : "off") + '">' + (x.IsActive ? "نشط" : "غير نشط") + '</span></td><td><div class="ep-row-actions"><button type="button" title="عرض" data-profile="' + x.EmployeeId + '"><i class="fas fa-user"></i></button><button type="button" title="تعديل" data-edit="' + x.EmployeeId + '"><i class="fas fa-edit"></i></button><button type="button" data-active="' + x.EmployeeId + '" data-state="' + (!x.IsActive) + '">' + (x.IsActive ? "تعطيل" : "تفعيل") + '</button></div></td></tr>');
            });
            renderEmployeeProfile(rows[0]);
            if (loadState) { loadState.textContent = "تم تحميل " + rows.length + " موظف"; }
            message("تم تحميل " + rows.length + " موظف");
        });
    }
    loadEmployees = loadEmployeesModern;
    ...
    function openEditor(employee) {
        ...
        rememberRecentEmployee(employee);
        updateInsuranceStateBar(employee);
    ...
    function renderInsuranceHistoryModern(rows) {
        var tbody = byId("epInsuranceHistory");
        if (!tbody) { return; }
        tbody.innerHTML = "";
        if (!(rows || []).length) {
            tbody.innerHTML = '<tr><td colspan="6" class="ep-empty-row">لا يوجد سجل سابق للتأمين الطبي لهذا الموظف.</td></tr>';
            return;
        }
        (rows || []).forEach(function (x) {
            tbody.insertAdjacentHTML("beforeend", '<tr><td>' + html(x.PlanName) + '</td><td>' + html(dateInput(x.StartDate)) + '</td><td>' + html(dateInput(x.EndDate)) + '</td><td>' + money(x.EmployeeMonthlyDeduction) + '</td><td>' + money(x.CompanyMonthlyCost) + '</td><td><span class="ep-state-chip ' + (x.IsActive ? "insured" : "inactive") + '">' + (x.IsActive ? "نشط" : "متوقف") + '</span></td></tr>');
        });
    }
    renderInsuranceHistory = renderInsuranceHistoryModern;
    ...
    function clientTerm(value) {
        var text = String(value == null ? "" : value);
        return text
            .replace(/historically inconsistent behavior/gi, "Historical posting pattern requires finance review")
            .replace(/missing VB6 branch/gi, "لا توجد قيود مرحلة مطابقة لهذا الفرع")
            .replace(/Reconstructed/g, "صف محسوب غير معتمد للترحيل")
            .replace(/reconstructed row/gi, "Calculated preview row - not approved for posting")
            .replace(/duplicate replayed line/gi, "Duplicate accounting preview line")
            .replace(/missing replayed line/gi, "Expected accounting preview line is not present")
            .replace(/amount mismatch/gi, "Amount requires finance review")
            .replace(/line count mismatch/gi, "Line count requires finance review");
    }
    function collectTestPostingRequest() {
        var request = salaryRequest();
        request.IncludeLineDetails = true;
        request.Password = byId("epTestPostingPassword") ? byId("epTestPostingPassword").value : "";
        request.ConfirmationPhrase = byId("epTestPostingPhrase") ? byId("epTestPostingPhrase").value : "";
        return request;
    }
    ...
    function compatibilityStatus(row) {
        var status = row.CompatibilityStatus || (row.IsLegacySnapshot ? "LegacySnapshot" : "Reconstructed");
        return status;
    }
    function compatibilityBadge(row) {
        var status = compatibilityStatus(row);
        if (status === "LegacySnapshot") {
            return '<span class="ep-compat-badge legacy">لقطة محفوظة</span>';
        }
        return '<span class="ep-compat-badge reconstructed">Calculated preview</span>';
    }
    function explainComponentNo(row) {
        var components = row.Components || [];
        for (var i = 0; i < components.length; i++) {
            if (number(components[i].FinalValue || components[i].Value || components[i].LegacyValue) !== 0) {
                return components[i].ComponentNo || components[i].CompNo || (i + 1);
            }
        }
        return 1;
    }
    function renderSalaryModern(preview) {
        preview = preview || {};
        var rows = preview.Rows || [];
        setText("epTotalBasic", money(preview.TotalBasic));
        setText("epTotalAdditions", money(preview.TotalAdditions));
        setText("epTotalDeductions", money(preview.TotalDeductions));
        ...
        var legacyRows = preview.LegacyRows != null ? preview.LegacyRows : rows.filter(function (x) { return compatibilityStatus(x) === "LegacySnapshot"; }).length;
        var reconstructedRows = preview.ReconstructedRows != null ? preview.ReconstructedRows : rows.length - legacyRows;
        setText("epRunRowsCount", preview.TotalRows || rows.length);
        setText("epLegacyRowsCount", legacyRows);
        setText("epReconstructedRowsCount", reconstructedRows);
        setText("epParityProgress", (preview.TotalRows || rows.length) ? Math.round((legacyRows / (preview.TotalRows || rows.length)) * 100) + "%" : "0%");
        setText("epPreviewStatus", reconstructedRows ? "المعاينة آمنة للعرض فقط: توجد صفوف محسوبة محمية من الترحيل." : "المعاينة مبنية على لقطات محفوظة وقابلة للمراجعة المحاسبية.");
        var salaryRows = byId("epSalaryRows");
        if (salaryRows) {
            salaryRows.innerHTML = "";
            if (!rows.length) {
                salaryRows.innerHTML = '<tr><td colspan="10" class="ep-empty-row">لا توجد بيانات مسير للفترة الحالية.</td></tr>';
            }
            var salaryHtml = rows.map(function (x) {
                var additions = number(x.SalaryAllowances) + number(x.VariableAdditions);
                var deductions = number(x.ExistingDiscounts) + number(x.AdvanceDeduction);
                var insurance = number(x.MedicalInsuranceDeduction) || number(x.TotalInsuranceLegacy);
                var componentNo = explainComponentNo(x);
                return '<tr><td><strong>' + html(x.EmployeeCode) + ' - ' + html(x.EmployeeName) + '</strong><small>' + html(x.DepartmentName || x.JobTypeName || "") + '</small></td><td>' + html(x.BranchName) + '</td><td>' + compatibilityBadge(x) + '</td><td>' + money(x.BasicSalary) + '</td><td>' + money(additions) + '</td><td>' + money(x.AdvanceDeduction) + '</td><td>' + money(deductions) + '</td><td>' + money(insurance) + '</td><td><strong>' + money(x.NetSalary) + '</strong></td><td><button type="button" class="ep-action-ghost" data-explain-employee="' + html(x.EmployeeId) + '" data-explain-component="' + html(componentNo) + '"><i class="fas fa-search-plus"></i> تفسير</button></td></tr>';
            }).join("");
            if (salaryHtml) {
                salaryRows.innerHTML = salaryHtml + (preview.PayloadIsTruncated ? '<tr><td colspan="10" class="ep-empty-row">تم عرض أول ' + rows.length + ' صف فقط لحماية سرعة المتصفح. استخدم فرع/قسم/موظف لتضييق المعاينة.</td></tr>' : "");
            }
        }
        var journalRows = byId("epJournalRows");
        if (journalRows) {
            journalRows.innerHTML = "";
            journalRows.innerHTML = (preview.JournalPreview || []).map(function (x) {
                return '<tr><td>' + html(x.AccountCode || "غير محدد") + '</td><td>' + money(x.Debit) + '</td><td>' + money(x.Credit) + '</td><td>' + html(x.Description) + '</td><td>' + html(x.EmployeeId || "") + '</td></tr>';
            }).join("");
            if (preview.PayloadIsTruncated && preview.TotalJournalPreviewRows > (preview.JournalPreview || []).length) {
                journalRows.innerHTML += '<tr><td colspan="5" class="ep-empty-row">تم عرض أول ' + (preview.JournalPreview || []).length + ' قيد من أصل ' + preview.TotalJournalPreviewRows + '.</td></tr>';
            }
            if (!journalRows.innerHTML) { journalRows.innerHTML = '<tr><td colspan="5" class="ep-empty-row">لا توجد قيود معاينة. الترحيل المحاسبي محمي حتى اكتمال parity.</td></tr>'; }
        }
        message(preview.Message || "تم حساب معاينة المسير بأمان");
    }
    renderSalary = renderSalaryModern;
    ...
    function renderParity(report) {
        report = report || {};
        var panel = byId("epParityPanel");
        if (panel) { panel.hidden = false; }
        setText("epParityNetDiff", money(report.NetDifference));
        setText("epRunRowsCount", report.TotalRows || 0);
        setText("epLegacyRowsCount", report.LegacySnapshotRows || 0);
        setText("epReconstructedRowsCount", report.ReconstructedRows || 0);
        setText("epParityProgress", report.TotalRows ? Math.round(((report.TotalRows - (report.TotalMismatchRows || 0)) / report.TotalRows) * 100) + "%" : "0%");
        var componentRows = byId("epParityComponentRows");
        if (componentRows) { componentRows.innerHTML = '<tr><td>Component diffs</td><td>' + html(report.ComponentMismatchRows || 0) + '</td><td>تعرض الفروقات حسب Comp1..Comp40 مع التصنيف semantic.</td></tr>'; }
        var totalRows = byId("epParityTotalRows");
        if (totalRows) { totalRows.innerHTML = '<tr><td>Total/Net diffs</td><td>' + html(report.TotalMismatchRows || 0) + '</td><td>الفروقات المتبقية قبل السماح بأي ترحيل.</td></tr>'; }
        setText("epParityComponentRows", report.ComponentMismatchRows || 0);
        setText("epParityTotalRows", report.TotalMismatchRows || 0);
        var categories = {};
        (report.Rows || []).forEach(function (row) { (row.ComponentDiffs || []).forEach(function (diff) { var key = diff.MismatchCategory || "unclassified"; categories[key] = (categories[key] || 0) + 1; }); });
        var categoryHost = byId("epParityCategories");
        if (categoryHost) {
            categoryHost.innerHTML = Object.keys(categories).map(function (key) { return '<span class="ep-category-chip">' + html(key) + '<b>' + categories[key] + '</b></span>'; }).join("") || '<span class="ep-category-chip">لا توجد تصنيفات نشطة<b>0</b></span>';
        }
        message("تم تحميل تقرير parity للمعاينة الحالية");
    }
    function loadParity() {
        var url = root.getAttribute("data-parity-url");
        if (!url) { message("مسار parity غير متاح", true); return Promise.resolve(); }
        message("جار تحميل تقرير parity...");
        return getJson(url + "?" + queryString(salaryRequest())).then(function (res) {
            if (!res.success) { message(res.message || "تعذر تحميل parity", true); return; }
            renderParity(res.report);
        });
    }
    function renderReplay(report) {
        report = report || {};
        var panel = byId("epReplayPanel");
        if (panel) { panel.hidden = false; }
        setText("epReplayLegacyDebit", money(report.LegacyDebitTotal));
        setText("epReplayDebit", money(report.ReplayedDebitTotal));
        setText("epReplayDebitDiff", money(report.DebitDifference));
        setText("epReplayLegacyCredit", money(report.LegacyCreditTotal));
        setText("epReplayCredit", money(report.ReplayedCreditTotal));
        setText("epReplayCreditDiff", money(report.CreditDifference));
        var body = byId("epReplayComparisons");
        if (body) {
            body.innerHTML = "";
            var comparisons = []
                .concat(report.AccountComparisons || [])
                .concat(report.BranchComparisons || [])
                .concat(report.DepartmentComparisons || [])
                .concat(report.ProjectComparisons || []);
            comparisons.slice(0, 80).forEach(function (x) {
                body.insertAdjacentHTML("beforeend", '<tr><td>' + html((x.Dimension || "Account") + ": " + (x.Key || "غير محدد")) + '</td><td>' + money(x.LegacyDebit) + '</td><td>' + money(x.ReplayDebit) + '</td><td>' + money(x.LegacyCredit) + '</td><td>' + money(x.ReplayCredit) + '</td><td>' + html(x.MismatchCategory || "") + '</td></tr>');
            });
            if (!body.innerHTML) { body.innerHTML = '<tr><td colspan="6" class="ep-empty-row">لا توجد مقارنات محاسبية للفترة الحالية.</td></tr>'; }
        }
        var categoryHost = byId("epParityCategories");
        if (categoryHost && report.DistributionMismatchCategories) {
            categoryHost.innerHTML = report.DistributionMismatchCategories.map(function (x) {
                return '<span class="ep-category-chip">' + html(x.Category) + '<b>' + html(x.Count || 0) + '</b></span>';
            }).join("");
        }
        message("تم تحميل replay المحاسبي للقراءة فقط");
    }
    var renderReplayLegacy = renderReplay;
    renderReplay = function (report) {
        renderReplayLegacy(report);
        report = report || {};
        var body = byId("epReplayComparisons");
        if (!body) { return; }
        var comparisons = []
        ...
        body.innerHTML = "";
        comparisons.slice(0, 80).forEach(function (x) {
                body.insertAdjacentHTML("beforeend", '<article class="ep-replay-card"><strong>' + html((x.Dimension || "الحساب") + ": " + (x.Key || "غير محدد")) + '</strong><span>مدين مرحل: ' + money(x.LegacyDebit) + ' / مدين المحاكاة: ' + money(x.ReplayDebit) + '</span><span>دائن مرحل: ' + money(x.LegacyCredit) + ' / دائن المحاكاة: ' + money(x.ReplayCredit) + '</span><em>' + html(x.MismatchCategory || "مصنف") + '</em></article>');
        });
        if (!body.innerHTML) {
            body.innerHTML = '<span class="ep-category-chip">No accounting replay comparisons<b>0</b></span>';
        }
    };
    renderReplay = function (report) {
        report = report || {};
        ...
        body.innerHTML = "";
        comparisons.slice(0, 80).forEach(function (x) {
                body.insertAdjacentHTML("beforeend", '<article class="ep-replay-card"><strong>' + html((x.Dimension || "الحساب") + ": " + (x.Key || "غير محدد")) + '</strong><span>مدين مرحل: ' + money(x.LegacyDebit) + ' / مدين المعاينة: ' + money(x.ReplayDebit) + '</span><span>دائن مرحل: ' + money(x.LegacyCredit) + ' / دائن المعاينة: ' + money(x.ReplayCredit) + '</span><em>' + html(clientTerm(x.MismatchCategory || "مصنف")) + '</em></article>');
        });
        if (!body.innerHTML) {
            body.innerHTML = '<span class="ep-category-chip">No accounting replay comparisons<b>0</b></span>';
        }
        message("تم تحميل المحاكاة المحاسبية كمعاينة محمية للقراءة فقط.");
    };

    function loadReplay() {
        var url = root.getAttribute("data-replay-url");
        if (!url) { message("مسار accounting replay غير متاح", true); return Promise.resolve(); }
        var request = salaryRequest();
        request.IncludeLineDetails = false;
        message("جار تحميل replay المحاسبي...");
        return getJson(url + "?" + queryString(request)).then(function (res) {
            if (!res.success) { message(res.message || "تعذر تحميل replay", true); return; }
            renderReplay(res.replay || res.report);
        });
    }
    function renderTestPosting(result) {
        result = result || {};
        setText("epTestPostingDatabase", result.DatabaseName || "--");
        setText("epTestPostingAllowed", result.IsAllowedDatabase ? "Allowed test database" : "Not allowlisted for test posting");
        setText("epTestPostingNotes", result.NotesCount || 0);
        setText("epTestPostingLines", result.VoucherLinesCount || 0);
        setText("epTestPostingBalance", money(result.Balance));
        if (result.TestPostingBatchId && byId("epTestPostingBatchId")) { byId("epTestPostingBatchId").value = result.TestPostingBatchId; }
        var warnings = byId("epTestPostingWarnings");
        if (warnings) {
            warnings.innerHTML = (result.Warnings || []).map(function (x) { return '<span><i class="fas fa-exclamation-triangle"></i> ' + html(clientTerm(x)) + '</span>'; }).join("");
            if (!warnings.innerHTML && result.Message) { warnings.innerHTML = '<span class="ok"><i class="fas fa-check-circle"></i> ' + html(result.Message) + '</span>'; }
        }
        var accounts = byId("epTestPostingAccounts");
        if (accounts) {
            accounts.innerHTML = (result.AffectedAccounts || []).slice(0, 10).map(function (x) {
                return '<article><strong>' + html(x.Key) + '</strong><span>D ' + money(x.Debit) + ' / C ' + money(x.Credit) + '</span><small>' + html(x.Lines) + ' lines</small></article>';
            }).join("") || '<div class="ep-empty-card">Run dry-run to see affected accounts.</div>';
        }
        var dimensions = byId("epTestPostingDimensions");
        if (dimensions) {
            var rows = []
                .concat((result.AffectedBranches || []).slice(0, 5).map(function (x) { x._t = "Branch"; return x; }))
                .concat((result.AffectedProjects || []).slice(0, 5).map(function (x) { x._t = "Project"; return x; }))
                .concat((result.AffectedDepartments || []).slice(0, 5).map(function (x) { x._t = "Department"; return x; }));
            dimensions.innerHTML = rows.map(function (x) {
                return '<article><strong>' + html(x._t + ": " + x.Key) + '</strong><span>D ' + money(x.Debit) + ' / C ' + money(x.Credit) + '</span><small>' + html(x.Lines) + ' lines</small></article>';
            }).join("") || '<div class="ep-empty-card">Run dry-run to see operational distribution.</div>';
        }
        message(result.Message || "معاينة الترحيل الاختباري المحمي جاهزة.", !result.IsAllowedDatabase);
    }
    function dryRunTestPosting() {
        var url = root.getAttribute("data-test-posting-dry-run-url");
        if (!url) { message("الترحيل الاختباري المحمي غير متاح في هذه المساحة.", true); return Promise.resolve(); }
        message("Preparing protected test posting dry-run...");
        return postJson(url, collectTestPostingRequest()).then(function (res) {
            if (!res.success) { message(res.message || "Dry-run failed.", true); return; }
            renderTestPosting(res.result);
        });
    }
    function generateTestPosting() {
        var url = root.getAttribute("data-test-posting-generate-url");
        if (!url) { message("إنشاء الترحيل الاختباري المحمي غير متاح في هذه المساحة.", true); return Promise.resolve(); }
        message("Generating protected test posting in the allowlisted test database...");
        return postJson(url, collectTestPostingRequest()).then(function (res) {
            if (!res.success) { message(res.message || "Test posting generation failed.", true); return; }
            renderTestPosting(res.result);
        });
    }
    function cleanupTestPosting() {
        var url = root.getAttribute("data-test-posting-cleanup-url");
        if (!url) { message("تنظيف الدفعة المحمية غير متاح في هذه المساحة.", true); return Promise.resolve(); }
        var batchId = byId("epTestPostingBatchId") ? byId("epTestPostingBatchId").value : "";
        if (!batchId) { message("Enter a TestPostingBatchId before cleanup.", true); return Promise.resolve(); }
        message("Cleaning protected test posting batch...");
        return postJson(url, {
            TestPostingBatchId: batchId,
            Password: byId("epTestPostingPassword") ? byId("epTestPostingPassword").value : "",
            ConfirmationPhrase: byId("epTestPostingPhrase") ? byId("epTestPostingPhrase").value : ""
        }).then(function (res) {
            if (!res.success) { message(res.message || "Cleanup failed.", true); return; }
            renderTestPosting(res.result);
            message("Cleanup completed. Notes: " + (res.result.CleanedNotesCount || 0) + ", lines: " + (res.result.CleanedVoucherLinesCount || 0));
        });
    }

    function openExplainDrawer() {
        var drawer = byId("epExplainDrawer");
        var backdrop = byId("epExplainBackdrop");
    ...
    function renderExplain(explain) {
        explain = explain || {};
        setText("epExplainTitle", (explain.EmployeeCode || "") + " - " + (explain.EmployeeName || "") + " / " + (explain.ComponentNameAr || ("Comp" + explain.ComponentNo)));
        var body = byId("epExplainBody");
        if (body) {
            var proration = explain.Proration || {};
            var insurance = explain.Insurance || {};
            var lines = explain.Explanation || [];
            body.innerHTML = '<div class="ep-explain-grid"><div><b>Legacy</b><strong>' + money(explain.LegacyValue) + '</strong></div><div><b>Reconstructed</b><strong>' + money(explain.ReconstructedValue) + '</strong></div><div><b>Diff</b><strong>' + money(explain.Difference) + '</strong></div><div><b>Confidence</b><strong>' + html(explain.ConfidenceScore || 0) + '%</strong></div></div><section><h4>سلسلة القرار</h4><p>' + html(explain.PrecedenceDecision || "لم يتم تحديد قرار precedence.") + '</p><p><span class="ep-category-chip">' + html(explain.MismatchCategory || "no mismatch") + '<b>' + html(explain.LikelySource || "deterministic") + '</b></span></p></section><section><h4>Proration / Temporal</h4><p>CountDays: ' + html(proration.CountDays || "") + ' · Denominator: ' + html(proration.ActualDenominator || proration.ExpectedDenominator || "") + ' · Rule: ' + html(proration.RulePath || "") + '</p><p>' + html(proration.DenominatorReason || "") + '</p></section><section><h4>Insurance impact</h4><p>Source: ' + html(insurance.SourceTables || insurance.SourceFunction || "") + ' · Basis: ' + html(insurance.InsuranceComponentBase || insurance.TBLInsurancesJoinBase || "") + ' · Posting: ' + html(insurance.PostingRule || "") + ' · Exclusion: ' + html(insurance.ExclusionReason || "لا يوجد") + '</p></section><section><h4>Trace</h4>' + (lines.length ? '<ol>' + lines.map(function (x) { return '<li>' + html(x) + '</li>'; }).join("") + '</ol>' : '<p>لا توجد trace lines من المحرك.</p>') + '</section>';
        }
        openExplainDrawer();
    }
    function explainEmployeeComponent(employeeId, componentNo) {
        var url = root.getAttribute("data-explain-url");
        if (!url) { message("مسار explainability غير متاح", true); return; }
        var request = salaryRequest();
        request.EmployeeId = employeeId || request.EmployeeId;
        request.ComponentNo = componentNo || 1;
        message("جار تحميل تفسير المكون...");
        return getJson(url + "?" + queryString(request)).then(function (res) {
            if (!res.success) { message(res.message || "تعذر تحميل التفسير", true); return; }
            renderExplain(res.explain || res.result || res);
        });
    }
    ...
    loadLookups().then(function () {
        if (screen === "employees") { renderRecentEmployees(); }
    ...
    root.addEventListener("click", function (e) {
        ...
        if (e.target && e.target.id === "epExplainBackdrop") {
            closeExplainDrawer();
            return;
        }
        ...
        if (btn.id === "epPrevMonth") { setRunMonthOffset(-1); }
        if (btn.id === "epNextMonth") { setRunMonthOffset(1); }
        if (btn.id === "epCurrentMonth") { setCurrentRunMonth(); }
        if (btn.id === "epLoadParity") { loadParity(); }
        if (btn.id === "epLoadReplay") { loadReplay(); }
        if (btn.id === "epTestPostingDryRun") { dryRunTestPosting(); }
        if (btn.id === "epGenerateTestPosting") { generateTestPosting(); }
        if (btn.id === "epCleanupTestPosting") { cleanupTestPosting(); }
        if (btn.id === "epCloseExplain") { closeExplainDrawer(); }
        if (btn.id === "epExplainBackdrop") { closeExplainDrawer(); }
        if (btn.hasAttribute("data-explain-employee")) {
            explainEmployeeComponent(parseInt(btn.getAttribute("data-explain-employee"), 10), parseInt(btn.getAttribute("data-explain-component") || "1", 10));
        }
        if (btn.hasAttribute("data-profile")) {
            var profile = (root._employeeRows || []).filter(function (x) { return String(x.EmployeeId) === btn.getAttribute("data-profile"); })[0];
            renderEmployeeProfile(profile || null);
            rememberRecentEmployee(profile || null);
        }
        if (btn.hasAttribute("data-recent-employee")) {
            var recentId = btn.getAttribute("data-recent-employee");
            getJson(root.getAttribute("data-get-url") + "?id=" + encodeURIComponent(recentId)).then(function (res) { openEditor(res.employee); });
        }
        if (btn.id === "epSaveRun") {
            message("Payroll posting remains protected until accounting parity validation is complete.", false);
            return;
        }
    ...
    document.addEventListener("keydown", function (e) {
        if ((e.ctrlKey || e.metaKey) && String(e.key).toLowerCase() === "k" && screen === "employees" && byId("epSearchTerm")) {
            e.preventDefault();
            byId("epSearchTerm").focus();
            byId("epSearchTerm").select();
        }
        if ((e.ctrlKey || e.metaKey) && e.key === "Enter" && screen === "salary" && byId("epPreviewRun")) {
            e.preventDefault();
            previewSalary();
        }
        if (e.altKey && String(e.key).toLowerCase() === "p" && screen === "salary") {
            e.preventDefault();
            loadParity();
        }
        if (e.altKey && String(e.key).toLowerCase() === "r" && screen === "salary") {
            e.preventDefault();
            loadReplay();
        }
        if (e.key === "Escape") {
            closeExplainDrawer();
            if (byId("epEmployeeEditor")) { byId("epEmployeeEditor").hidden = true; }
        }
    });
