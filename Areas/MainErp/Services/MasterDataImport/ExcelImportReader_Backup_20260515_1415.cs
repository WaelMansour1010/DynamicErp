    ...
    public class ExcelImportReader
    {
            ...
            foreach (var file in filePathsByOriginalName)
            {
                ...
                foreach (var sheet in ReadWorksheets(file.Value))
                {
                    ...
                    for (var i = 0; i < sheet.Table.Rows.Count; i++)
                    {
                        ...
                        var entryDateText = Get(dataRow, "Entry Date", "Date", "تاريخ القيد", "تاريخ");
                        var referenceNo = Get(dataRow, "Reference No", "ReferenceNo", "Ref No", "رقم المرجع");
                        var entryNo = Get(dataRow, "Entry No", "EntryNo", "رقم القيد");
                        var voucherNo = Get(dataRow, "Voucher No", "VoucherNo", "رقم السند", "رقم الحركة");
                        var debitText = Get(dataRow, "Debit", "مدين");
                        var creditText = Get(dataRow, "Credit", "دائن");
                        var row = new JournalEntryImportRowViewModel
                        {
                            ...
                            AccountSerial = Get(dataRow, "Account Serial", "Account_Serial", "مسلسل الحساب", "رقم الحساب", "كود الحساب"),
                            AccountName = Get(dataRow, "Account Name", "اسم الحساب", "الحساب"),
                            DebitText = debitText,
                            CreditText = creditText,
                            Debit = ParseDecimal(debitText).GetValueOrDefault(),
                            Credit = ParseDecimal(creditText).GetValueOrDefault(),
                            Description = Get(dataRow, "Description", "البيان", "بيان", "Remarks", "ملاحظات"),
                            Branch = Get(dataRow, "Branch", "الفرع"),
                            CostCenter = Get(dataRow, "Cost Center", "CostCenter", "مركز التكلفة"),
            ...
            foreach (var file in filePathsByOriginalName)
            {
                ...
                foreach (var sheet in ReadWorksheets(file.Value))
                {
                    ...
                    for (var i = 0; i < sheet.Table.Rows.Count; i++)
                    {
                        ...
                        var debitText = Get(dataRow, "Debit", "مدين");
                        var creditText = Get(dataRow, "Credit", "دائن");
                        var balanceText = Get(dataRow, "Opening Balance", "OpeningBalance", "Balance", "الرصيد الافتتاحي", "رصيد افتتاحي", "الرصيد");
                        var row = new JournalEntryImportRowViewModel
                        {
                            FileName = file.Key,
                            SheetName = sheet.Name,
                            ...
                            EntryDateText = Get(dataRow, "Entry Date", "Date", "Opening Date", "تاريخ القيد", "تاريخ الرصيد", "تاريخ"),
                            AccountSerial = Get(dataRow, "Account Serial", "Account_Serial", "مسلسل الحساب", "رقم الحساب", "كود الحساب"),
                            AccountName = Get(dataRow, "Account Name", "اسم الحساب", "الحساب"),
                            DebitText = debitText,
                            CreditText = creditText,
                            OpeningBalanceText = balanceText,
                            BalanceType = Get(dataRow, "Balance Type", "BalanceType", "طبيعة الرصيد", "طبيعة"),
                            Description = Get(dataRow, "Notes", "Description", "البيان", "ملاحظات"),
                            Branch = Get(dataRow, "Branch", "الفرع"),
                            CostCenter = Get(dataRow, "Cost Center", "CostCenter", "مركز التكلفة")
                        };
        ...
        private static void ReadLegacyOpeningBalanceSheet(IList<JournalEntryImportRowViewModel> rows, string fileName, string fileHash, WorksheetData sheet)
        {
            for (var i = 0; i < sheet.Table.Rows.Count; i++)
            {
                ...
                var serial = GetByIndex(dataRow, 7);
                var name = GetByIndex(dataRow, 6);
                if (IsLikelyHeader(serial, name) || string.IsNullOrWhiteSpace(serial) || string.IsNullOrWhiteSpace(name))
                {
                    continue;
                }

                var balanceText = GetByIndex(dataRow, 5);
                var balance = ParseSignedBalance(balanceText);
                if (!balance.HasValue || balance.Value == 0m)
                {
                    continue;
                }

                var balanceLabel = FirstNonEmpty(GetByIndex(dataRow, 4), GetByIndex(dataRow, 0));
                var row = new JournalEntryImportRowViewModel
                {
                    FileName = fileName,
                    SheetName = sheet.Name,
                ...
                ApplyLegacyOpeningBalanceSide(row, balance.Value, balanceLabel);
                rows.Add(row);
            }
        }
