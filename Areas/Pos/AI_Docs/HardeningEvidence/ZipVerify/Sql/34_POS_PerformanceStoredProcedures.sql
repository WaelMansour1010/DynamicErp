...
BEGIN
    ...
    BEGIN
        SELECT
            CASE WHEN a.AccountTab = 2 THEN N'Ø§Ù„Ø¥ÙŠØ±Ø§Ø¯Ø§Øª' WHEN a.AccountTab = 3 THEN N'Ø§Ù„Ù…ØµØ±ÙˆÙØ§Øª' ELSE N'Ø£Ø®Ø±Ù‰' END AS SectionName,
            a.Account_Serial AS AccountSerial,
            a.Account_Code AS AccountCode,
            a.Account_Name AS AccountName,
...
BEGIN
    ...
    SELECT TOP (200)
        ...
        COALESCE(NULLIF(b.branch_name, N''), NULLIF(b.branch_namee, N''), N'ÙØ±Ø¹ ' + CONVERT(NVARCHAR(20), n.branch_no)) AS BranchName,
...
BEGIN
    ...
    SELECT TOP (100)
        ...
        CASE WHEN ISNULL(n.CashingType, 0) = 5 THEN N'Ø§Ø³ØªØ¹Ø§Ø¶Ø© Ø¹Ù‡Ø¯Ø©' WHEN ISNULL(n.CashingType, 0) = 6 THEN N'ØªÙ…ÙˆÙŠÙ„ Ø®Ø²ÙŠÙ†Ø©' ELSE CONVERT(NVARCHAR(20), n.CashingType) END AS CashingTypeName,
...
BEGIN
    ...
    ELSE
    BEGIN
        SET @statusMessage = N'áÇ ÊæÌÏ ÕáÇÍíÉ ßÇÝíÉ áÞÑÇÁÉ ãÄÔÑÇÊ ÇáÎÇÏã. íÊØáÈ åÐÇ ÇáÌÒÁ ÕáÇÍíÉ VIEW SERVER STATE.';
...
/*
    ...
    The candidate reporting indexes were tested on a representative POS database with a 120-worker mixed load
