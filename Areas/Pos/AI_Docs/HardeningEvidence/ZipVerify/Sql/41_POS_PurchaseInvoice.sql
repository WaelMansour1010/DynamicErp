...
BEGIN
    ...
    BEGIN
        RAISERROR(N'المورد مطلوب', 16, 1);
        RETURN;
    END

    IF @BranchId IS NULL OR @BranchId <= 0
    BEGIN
        RAISERROR(N'الفرع مطلوب', 16, 1);
        RETURN;
    END

    IF @StoreId IS NULL OR @StoreId <= 0
    BEGIN
        RAISERROR(N'المخزن مطلوب', 16, 1);
        RETURN;
    END

    IF NOT EXISTS (SELECT 1 FROM @Items)
    BEGIN
        RAISERROR(N'يجب إضافة صنف واحد على الأقل', 16, 1);
        RETURN;
    END
    ...
    BEGIN
        RAISERROR(N'يوجد صنف غير موجود', 16, 1);
        RETURN;
    END
    ...
    (
        SELECT 1
        FROM @Items i
        WHERE i.UnitId IS NOT NULL
          AND NOT EXISTS
          (
              SELECT 1
              FROM dbo.TblItemsUnits iu
              WHERE iu.ItemID = i.ItemId
                AND iu.UnitID = i.UnitId
          )
    )
    BEGIN
        RAISERROR(N'يوجد صنف بوحدة غير صحيحة', 16, 1);
        RETURN;
    END
    ...
    (
        SELECT 1
        FROM @Items i
        INNER JOIN dbo.TblItems ti ON ti.ItemID = i.ItemId
        WHERE ISNULL(ti.HaveSerial, 0) = 1
          AND NULLIF(LTRIM(RTRIM(i.ItemSerial)), N'') IS NULL
    )
    BEGIN
        RAISERROR(N'السيريال مطلوب للأصناف المسلسلة', 16, 1);
        RETURN;
    END
    ...
    BEGIN
        RAISERROR(N'كمية الصنف المسلسل يجب أن تكون 1', 16, 1);
        RETURN;
    END
    ...
    (
        SELECT 1
        FROM @Items i
        WHERE NULLIF(LTRIM(RTRIM(i.ItemSerial)), N'') IS NOT NULL
        GROUP BY UPPER(LTRIM(RTRIM(i.ItemSerial)))
        HAVING COUNT(1) > 1
    )
    BEGIN
        RAISERROR(N'يوجد سيريال مكرر في نفس الفاتورة', 16, 1);
        RETURN;
    END
    ...
    BEGIN
        ...
        BEGIN
            RAISERROR(N'لم يتم تحديد حساب الخزنة', 16, 1);
            RETURN;
        END
    END
    ...
    BEGIN
        ...
        IF @paymentAccount IS NULL
        BEGIN
            RAISERROR(N'لم يتم تحديد حساب البنك', 16, 1);
            RETURN;
        END
    END
    ELSE
    ...
    IF @lineVat > 0 AND @vatAccount IS NULL
    BEGIN
    ...
    (
        ...
        Vat, Vatyo, StoreID2, ItemSerial, OrderArrivalDate
    )
    SELECT
        @transactionId,
        i.ItemId,
        ...
        @StoreId,
        NULLIF(LTRIM(RTRIM(i.ItemSerial)), N''),
        @InvoiceDate
    FROM @Items i
    LEFT JOIN dbo.TblItems ti ON ti.ItemID = i.ItemId
    OUTER APPLY
