...
namespace MyERP.Areas.Pos.Data
{
    public class PosFinancialIntelligenceRepository
    {
        ...
        public PosFinancialIntelligenceResult GetCfoDashboard(PosFinancialIntelligenceFilter filter, int? lockedBranchId)
        {
            return Execute("dbo.usp_POS_FI_CfoDashboard", filter, lockedBranchId, true, true);
        }

        public PosFinancialIntelligenceResult GetBranchPerformance(PosFinancialIntelligenceFilter filter, int? lockedBranchId)
        {
            return Execute("dbo.usp_POS_FI_BranchPerformance", filter, lockedBranchId, false, false);
        }

        public PosFinancialIntelligenceResult GetCashFlowAnalysis(PosFinancialIntelligenceFilter filter, int? lockedBranchId)
        {
            return Execute("dbo.usp_POS_FI_CashFlowAnalysis", filter, lockedBranchId, false, false);
        }

        public PosFinancialIntelligenceResult GetSalesCollectionsAnalysis(PosFinancialIntelligenceFilter filter, int? lockedBranchId)
        {
            return Execute("dbo.usp_POS_FI_SalesCollectionsAnalysis", filter, lockedBranchId, false, false);
        }

        public PosFinancialIntelligenceResult GetExpenseAnalysis(PosFinancialIntelligenceFilter filter, int? lockedBranchId)
        {
            return Execute("dbo.usp_POS_FI_ExpenseAnalysis", filter, lockedBranchId, false, false);
        }

        public PosFinancialIntelligenceResult GetInventoryProfitability(PosFinancialIntelligenceFilter filter, int? lockedBranchId)
        {
            return Execute("dbo.usp_POS_FI_InventoryProfitability", filter, lockedBranchId, false, false);
        }
        ...
        public PosFinancialIntelligenceResult GetJournalDetails(PosFinancialIntelligenceDrilldownRequest request, int? lockedBranchId)
        {
            request = request ?? new PosFinancialIntelligenceDrilldownRequest();

            using (var connection = new SqlConnection(_connectionString))
            using (var command = new SqlCommand("dbo.usp_POS_FI_JournalDetails", connection))
            using (var adapter = new SqlDataAdapter(command))
            {
                command.CommandType = CommandType.StoredProcedure;
                command.CommandTimeout = 60;
                command.Parameters.Add("@DoubleEntryVoucherId", SqlDbType.Int).Value = DbValue(request.DoubleEntryVoucherId);
                command.Parameters.Add("@NotesId", SqlDbType.Int).Value = DbValue(request.NotesId);
                command.Parameters.Add("@TransactionId", SqlDbType.Int).Value = DbValue(request.TransactionId);
                command.Parameters.Add("@AccountCode", SqlDbType.NVarChar, 50).Value = DbValue(request.AccountCode);
        ...
        private PosFinancialIntelligenceResult Execute(string procedureName, PosFinancialIntelligenceFilter filter, int? lockedBranchId, bool includeReceivableParent, bool includeCustodyParent)
        {
            ...
            using (var adapter = new SqlDataAdapter(command))
            {
                ...
                if (procedureName.EndsWith("AccountingHealthDashboard", StringComparison.OrdinalIgnoreCase)
                    || procedureName.EndsWith("CfoDashboard", StringComparison.OrdinalIgnoreCase)
