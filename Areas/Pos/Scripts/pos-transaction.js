(function () {
    ...
    var resumeSaveAfterCommission = false;
    ...
    function uxClearGuide(message) {
        var validation = byId("validationSummary");
        if (validation && (!message || validation.innerText === message)) {
    ...
    function ensureCommissionReadyForSave() {
        ...
        if (commissionCalculationPending) {
            resumeSaveAfterCommission = true;
            byId("validationSummary").innerText = "جاري حساب القيمة، سيتم استكمال الحفظ تلقائياً بعد الانتهاء.";
            return false;
        }
    ...
    function readRuleList(name) {
        if (!commissionRules) { return []; }
        return commissionRules[name] || commissionRules[name.charAt(0).toUpperCase() + name.slice(1)] || [];
    }

    function readRuleObject(name) {
        if (!commissionRules) { return null; }
        return commissionRules[name] || commissionRules[name.charAt(0).toUpperCase() + name.slice(1)] || null;
    }
    ...
    function fieldValue(source, name) {
        ...
        return source[pascal] !== undefined ? source[pascal] : null;
    }

    function numericField(source, name) {
        var value = fieldValue(source, name);
        return value === null || value === undefined || value === "" ? 0 : (parseFloat(value) || 0);
    }

    function nullableNumericField(source, name) {
        var value = fieldValue(source, name);
        return value === null || value === undefined || value === "" ? null : (parseFloat(value) || 0);
    }
    ...
    function matchingRange(ranges, amount, itemId) {
        var rounded = Math.round(parseFloat(amount) || 0);
        for (var i = 0; i < ranges.length; i++) {
            var range = ranges[i];
            if (itemId && parseInt(fieldValue(range, "itemID") || fieldValue(range, "itemId"), 10) !== parseInt(itemId, 10)) { continue; }
            var from = numericField(range, "fromPrice");
            var to = numericField(range, "toPrice");
            if (rounded >= from && rounded <= to) {
                return range;
            }
        }
        return null;
    ...
    function itemCommissionRule(itemId) {
        var items = readRuleList("itemCommissions");
        for (var i = 0; i < items.length; i++) {
            if (parseInt(fieldValue(items[i], "itemID") || fieldValue(items[i], "itemId"), 10) === parseInt(itemId, 10)) {
                return items[i];
            }
        }
        return null;
    ...
    function hasSalesCommissionRule(item) {
        return item && (nullableNumericField(item, "percentVisa") !== null || nullableNumericField(item, "minVisa") !== null || nullableNumericField(item, "maxVisa") !== null);
    }

    function buildLocalCommissionResult(request, commission, vatPercent, source, percent, min, max, includeRechargeInTotal, bankMachineCommission) {
        var serviceFee = roundMoney(commission);
        var vatValue = roundMoney(serviceFee * (parseFloat(vatPercent) || 0) / 100);
        var totalFees = serviceFee + vatValue;
        var recharge = parseFloat(request.RechargeValue) || 0;
        var totalValue = includeRechargeInTotal === false ? totalFees : recharge + totalFees;
        var machineCommission = roundMoney(bankMachineCommission || 0);
        var machineWithdrawal = roundMoney(totalValue - machineCommission);
        return {
            ...
            BankMachineCommission: machineCommission,
            CashOutMachineWithdrawalAmount: machineWithdrawal < 0 ? 0 : machineWithdrawal,
            Source: source,
    ...
    function calculateLocalCommission(request) {
        if (!commissionRules || !request || !request.ItemID) { return null; }
        var mode = (request.ServiceType || "").toLowerCase();
        if (mode !== "cash-in" && mode !== "cash-out") { return null; }

        var amount = parseFloat(request.RechargeValue) || 0;
        var itemId = parseInt(request.ItemID, 10) || 0;
        var itemRanges = readRuleList("itemRanges");
        var itemRange = matchingRange(itemRanges, amount, itemId);
        var itemRule = itemCommissionRule(itemId);
        var systemOptions = readRuleObject("systemOptions") || {};

        if (mode === "cash-out") {
            var bankCommission = itemRange ? numericField(itemRange, "cost") : 0;
            var itemRangePrice = itemRange ? numericField(itemRange, "price") : 0;
            if (itemRange && itemRangePrice > 0) {
                return buildLocalCommissionResult(request, itemRangePrice > 0 && itemRangePrice < 1 ? amount * itemRangePrice : itemRangePrice, 0, "local tblItemsCash", 0, 0, 0, true, bankCommission);
            }

            var percent = hasSalesCommissionRule(itemRule) ? numericField(itemRule, "percentVisa") : numericField(systemOptions, "percentVisa");
            var min = hasSalesCommissionRule(itemRule) ? numericField(itemRule, "minVisa") : numericField(systemOptions, "minVisa");
            var max = hasSalesCommissionRule(itemRule) ? numericField(itemRule, "maxVisa") : numericField(systemOptions, "maxVisa");
            return buildLocalCommissionResult(request, applyMinMax(amount * percent / 100, min, max), 0, "local cash-out commission", percent, min, max, true, bankCommission);
        }
        ...
        var cashInItemRangePrice = itemRange ? numericField(itemRange, "price") : 0;
        if (cashInItemRangePrice > 0) {
            return buildLocalCommissionResult(request, cashInItemRangePrice > 0 && cashInItemRangePrice < 1 ? amount * cashInItemRangePrice : cashInItemRangePrice, request.Vatyo || 14, "local tblItemsCash", 0, 0, 0, true, 0);
        }

        var cashInPercent = hasSalesCommissionRule(itemRule) ? numericField(itemRule, "percentVisa") : numericField(systemOptions, "percentVisa");
    ...
    function calculateCommissionPreview() {
        ...
        if (commissionCache[key]) {
            ...
            setCommissionStatus("");
            uxClearGuide("جاري حساب القيمة...");
            uxClearGuide("جاري حساب القيمة، سيتم استكمال الحفظ تلقائياً بعد الانتهاء.");
            uxAdvanceAfterAmountCommit();
            if (resumeSaveAfterCommission) {
                resumeSaveAfterCommission = false;
                window.setTimeout(submitConfirmedSave, 0);
            }
            return;
        }
