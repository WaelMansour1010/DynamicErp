    ...
    public class PosExcelImportParser
    {
        ...
        private static bool IsTokenEligibleRow(PosExcelImportRowPreview row)
        {
            return row != null
                ...
                && (string.Equals((row.InternalServiceType ?? string.Empty).Trim(), "cash-in", StringComparison.OrdinalIgnoreCase)
                    || NormalizeServiceText(row.ServiceType).Contains("كاش ان"));
        }
