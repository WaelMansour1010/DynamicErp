/*
    101_POS_TransactionIdAllocator_Hotfix.sql

    Purpose:
      Surgical hotfix for production POS save deadlocks in Transactions.Transaction_ID allocation.
      Recreates dbo.usp_POS_SaveTransaction from Phase 1 with only Transaction_ID allocation changed.

      Does NOT change Voucher_coding, NoteSerial, Sanad_No, accounting posting, invoice serials,
      branch/month/year serial scope, or business-visible numbering.

    Changes:
      - Captures the current dbo.usp_POS_SaveTransaction definition for rollback.
      - Sets Kishny POS voucher serial scope to Branch when script 99 is present.
      - Recreates dbo.usp_POS_SaveTransaction with stable branch/account/settings
        lookups before BEGIN TRANSACTION.
      - Preserves synchronous accounting, voucher coding meaning, issue voucher
        creation, ID allocation, and DEV_Serial cheap display assignment.

    Rollback:
      - Fast voucher-scope rollback:
            UPDATE dbo.TblOptions SET POSVoucherSerialScope = N'Company';
      - Procedure rollback:
            Reapply Areas/Pos/Sql/30_POS_SaveTransaction_UnicodeText.sql from
            the previous release, or restore the captured definition from
            dbo.POS_SaveTransactionPhase1Rollback.

    SQL Server compatibility: SQL Server 2008+ / 2012+ compatible. No CREATE SEQUENCE.
*/
...
BEGIN
    RAISERROR('Refusing to run POS save mitigation in a system database.', 16, 1);
    SET NOEXEC ON;
END;
GO

IF OBJECT_ID(N'dbo.POS_SaveTransactionPhase1Rollback', N'U') IS NULL
BEGIN
    CREATE TABLE dbo.POS_SaveTransactionPhase1Rollback
    (
        BackupId INT IDENTITY(1,1) NOT NULL CONSTRAINT PK_POS_SaveTransactionPhase1Rollback PRIMARY KEY,
        CreatedAt DATETIME NOT NULL CONSTRAINT DF_POS_SaveTransactionPhase1Rollback_CreatedAt DEFAULT(GETDATE()),
        DatabaseName SYSNAME NOT NULL,
        ProcedureName SYSNAME NOT NULL,
        ChangeName NVARCHAR(200) NOT NULL,
...
(
    SELECT 1
    FROM dbo.POS_SaveTransactionPhase1Rollback
    WHERE ProcedureName = N'usp_POS_SaveTransaction'
      AND ChangeName = N'Phase1_LockWindow_20260515'
)
BEGIN
    INSERT INTO dbo.POS_SaveTransactionPhase1Rollback
    (
        DatabaseName,
        ProcedureName,
        ChangeName,
    ...
    SELECT
        DB_NAME(),
        N'usp_POS_SaveTransaction',
        N'Phase1_LockWindow_20260515',
        OBJECT_DEFINITION(OBJECT_ID(N'dbo.usp_POS_SaveTransaction')),
        N'Before Phase 1 POS save lock-window mitigation.';
END;
GO
...
(
    ...
    FROM dbo.POS_SaveTransactionPhase1Rollback
    WHERE ProcedureName = N'usp_POS_SaveTransaction'
      AND ChangeName = N'TransactionIdAllocator_Hotfix_20260516'
)
BEGIN
    INSERT INTO dbo.POS_SaveTransactionPhase1Rollback
    (
    ...
    SELECT
        DB_NAME(),
        N'usp_POS_SaveTransaction',
        N'TransactionIdAllocator_Hotfix_20260516',
        OBJECT_DEFINITION(OBJECT_ID(N'dbo.usp_POS_SaveTransaction')),
        N'Before POS Transaction_ID allocator hotfix.';
END;
GO
...
IF @SeedLockResult < 0
BEGIN
    RAISERROR('Unable to acquire Transaction_ID allocator seed lock.', 16, 1);
END;
ELSE
BEGIN
    IF NOT EXISTS (SELECT 1 FROM dbo.POS_TransactionIdAllocator WITH (UPDLOCK, HOLDLOCK) WHERE CounterName = N'Transactions.Transaction_ID')
    BEGIN
...
BEGIN
    ...
    BEGIN
        BEGIN TRY
            ...
            IF @@ROWCOUNT <> 1 OR @NextValue IS NULL
            BEGIN
    ...
    SET @ErrorMsg = N'Unable to allocate Transactions.Transaction_ID after limited retries.';
    SET @NextValue = NULL;
    RETURN -1;
END;
GO
    ...
    BEGIN TRY
        ...
        */
        ...
        BEGIN
            ...
            */
            ...
            EXEC dbo.usp_POS_AllocateTransactionId
                @NextValue = @NextTransactionID OUTPUT,
                @ErrorMsg = @NextIDError OUTPUT;

            INSERT INTO @AllocationStages(StageName, DurationMs, Detail)
            VALUES (N'Transaction_ID allocation', DATEDIFF(MILLISECOND, @StageStart, GETDATE()), N'dbo.usp_POS_AllocateTransactionId:Transactions.Transaction_ID');

            IF @NextIDError IS NOT NULL OR @NextTransactionID IS NULL
            BEGIN
                SET @AllocationError = N'Unable to allocate Transaction_ID. Source=dbo.usp_POS_AllocateTransactionId; Table=dbo.Transactions; Field=Transaction_ID; Error=' + ISNULL(@NextIDError, N'<null>') + N'; NextValue=' + ISNULL(CONVERT(NVARCHAR(50), @NextTransactionID), N'<null>');
                ...
                INSERT INTO @AllocationStages(StageName, DurationMs, Detail, Success, ErrorNumber, ErrorMessage)
                VALUES (N'Transaction_ID allocation fallback', DATEDIFF(MILLISECOND, @StageStart, GETDATE()), N'Fallback=dbo.GetNextID_FromSequence:Transactions.Transaction_ID; PrimaryError=' + LEFT(@AllocationError, 250), CASE WHEN @NextIDError IS NULL AND @NextTransactionID IS NOT NULL THEN 1 ELSE 0 END, CASE WHEN @NextIDError IS NULL AND @NextTransactionID IS NOT NULL THEN NULL ELSE 50000 END, @NextIDError);

                IF @NextIDError IS NOT NULL OR @NextTransactionID IS NULL
                BEGIN
                    SET @AllocationError = @AllocationError + N'; FallbackError=' + ISNULL(@NextIDError, N'<null>') + N'; FallbackNextValue=' + ISNULL(CONVERT(NVARCHAR(50), @NextTransactionID), N'<null>');
        ...
        */
        ...
        BEGIN
            ...
            INSERT INTO @AllocationStages(StageName, DurationMs, Detail)
            VALUES (N'Issue voucher Transaction_ID allocation', DATEDIFF(MILLISECOND, @StageStart, GETDATE()), N'dbo.usp_POS_AllocateTransactionId:Transactions.Transaction_ID;StoreID=' + ISNULL(CONVERT(NVARCHAR(20), @IssueStoreID), N'<null>'));

            IF @NextIDError IS NOT NULL OR @IssueNextTransactionID IS NULL
            BEGIN
                SET @AllocationError = N'Unable to allocate Issue Voucher Transaction_ID. Source=dbo.usp_POS_AllocateTransactionId; Table=dbo.Transactions; Field=Transaction_ID; Error=' + ISNULL(@NextIDError, N'<null>') + N'; NextValue=' + ISNULL(CONVERT(NVARCHAR(50), @IssueNextTransactionID), N'<null>');
                ...
                INSERT INTO @AllocationStages(StageName, DurationMs, Detail, Success, ErrorNumber, ErrorMessage)
                VALUES (N'Issue voucher Transaction_ID allocation fallback', DATEDIFF(MILLISECOND, @StageStart, GETDATE()), N'Fallback=dbo.GetNextID_FromSequence:Transactions.Transaction_ID;StoreID=' + ISNULL(CONVERT(NVARCHAR(20), @IssueStoreID), N'<null>') + N'; PrimaryError=' + LEFT(@AllocationError, 180), CASE WHEN @NextIDError IS NULL AND @IssueNextTransactionID IS NOT NULL THEN 1 ELSE 0 END, CASE WHEN @NextIDError IS NULL AND @IssueNextTransactionID IS NOT NULL THEN NULL ELSE 50000 END, @NextIDError);

                IF @NextIDError IS NOT NULL OR @IssueNextTransactionID IS NULL
                BEGIN
                    SET @AllocationError = @AllocationError + N'; FallbackError=' + ISNULL(@NextIDError, N'<null>') + N'; FallbackNextValue=' + ISNULL(CONVERT(NVARCHAR(50), @IssueNextTransactionID), N'<null>');
                    RAISERROR(@AllocationError, 16, 1);
                END;
            END;
