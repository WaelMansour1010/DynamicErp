/*
    120_POS_VoucherCoding_AutoExpandTailDigits.sql

    Purpose:
      Recreate dbo.usp_GetNextSerial_V2 with the existing POS voucher serial scope
      behavior plus:
      - stale-counter self-heal path
      - automatic tail digit expansion when the configured digit width is exhausted

      If SerialCounters_V2 is behind actual Transactions.NoteSerial1 data for the
      same branch/type/month scope, the procedure reseeds that single counter forward
      during allocation and returns the next safe number instead of failing with:
        Unable to generate NoteSerial1 using dbo.usp_Voucher_coding_V2.

      If no_of_digit = 3 and the next tail is 1000, the serial becomes ...1000.
      Existing serials up to ...999 remain unchanged; no leading zero is added to
      old numbers and sanad_numbering.no_of_digit does not need to be widened.
    ...
    Rules:
      ...
      - Treat no_of_digit as the minimum padding width, not a hard maximum.
      ...
      - Does not run a broad data reseed during deployment; deployment should be fast.
...
BEGIN
    ...
    DECLARE @EffectiveNoOfDigits TINYINT;
    ...
    BEGIN TRY
        ...
        SET @EffectiveNoOfDigits = @NoOfDigits;
        ...
        */
        ...
        SET @EffectiveNoOfDigits =
            CASE
                WHEN LEN(CAST(@NewTail AS VARCHAR(20))) > @NoOfDigits THEN LEN(CAST(@NewTail AS VARCHAR(20)))
                ELSE @NoOfDigits
            END;

        DECLARE @BranchCode VARCHAR(10) = RIGHT(REPLICATE('0', 3) + CAST(@BranchID AS VARCHAR(3)), @BranchDigit);
        DECLARE @TailFormatted VARCHAR(20) = RIGHT(REPLICATE('0', 20) + CAST(@NewTail AS VARCHAR(20)), @EffectiveNoOfDigits);
        ...
        BEGIN
            ...
            BEGIN
                ...
                SET @TailFormatted = RIGHT(REPLICATE('0', 20) + CAST(@NewTail AS VARCHAR(20)), @EffectiveNoOfDigits);
