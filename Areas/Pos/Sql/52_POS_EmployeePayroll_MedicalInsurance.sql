-- POS Web medical-insurance master data, payroll deduction audit, and reporting support.
-- SQL Server 2012 compatible. Keep objects additive and idempotent for Cash deployments.
...
IF COL_LENGTH('dbo.MedicalInsuranceProviders', 'UpdatedAt') IS NULL ALTER TABLE dbo.MedicalInsuranceProviders ADD UpdatedAt DATETIME NULL;
IF COL_LENGTH('dbo.MedicalInsuranceProviders', 'UpdatedBy') IS NULL ALTER TABLE dbo.MedicalInsuranceProviders ADD UpdatedBy INT NULL;
GO
...
BEGIN
    ...
    (
        ...
        CoveragePercent MONEY NOT NULL CONSTRAINT DF_EmployeeMedicalInsurance_CoveragePercent DEFAULT (100),
...
IF COL_LENGTH('dbo.EmployeeMedicalInsurance', 'PolicyNumber') IS NULL ALTER TABLE dbo.EmployeeMedicalInsurance ADD PolicyNumber NVARCHAR(100) NULL;
IF COL_LENGTH('dbo.EmployeeMedicalInsurance', 'CardNumber') IS NULL ALTER TABLE dbo.EmployeeMedicalInsurance ADD CardNumber NVARCHAR(100) NULL;
IF COL_LENGTH('dbo.EmployeeMedicalInsurance', 'CoveragePercent') IS NULL ALTER TABLE dbo.EmployeeMedicalInsurance ADD CoveragePercent MONEY NOT NULL CONSTRAINT DF_EmployeeMedicalInsurance_CoveragePercent DEFAULT (100);
...
SET MonthlyCost = CASE WHEN MonthlyCost = 0 THEN Amount ELSE MonthlyCost END,
    ...
    END,
    CoveragePercent = CASE WHEN ISNULL(CoveragePercent, 0) = 0 THEN 100 ELSE CoveragePercent END
WHERE MonthlyCost = 0 OR EmployeeMonthlyDeduction = 0 OR ISNULL(CoveragePercent, 0) = 0;
GO
...
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = N'IX_EmployeeMedicalInsurance_Emp_Period' AND object_id = OBJECT_ID(N'dbo.EmployeeMedicalInsurance'))
    CREATE INDEX IX_EmployeeMedicalInsurance_Emp_Period ON dbo.EmployeeMedicalInsurance (EmpId, IsActive, IsMonthly, StartDate, EndDate);
GO
...
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = N'UX_PayrollMedicalInsuranceDeduction_Emp_Period' AND object_id = OBJECT_ID(N'dbo.PayrollMedicalInsuranceDeduction'))
    CREATE UNIQUE INDEX UX_PayrollMedicalInsuranceDeduction_Emp_Period ON dbo.PayrollMedicalInsuranceDeduction (EmpId, [Year], [Month]);
GO
...
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = N'UX_SalaryRunMedicalInsuranceDeduction_Emp_Period' AND object_id = OBJECT_ID(N'dbo.SalaryRunMedicalInsuranceDeduction'))
    CREATE UNIQUE INDEX UX_SalaryRunMedicalInsuranceDeduction_Emp_Period ON dbo.SalaryRunMedicalInsuranceDeduction (EmpId, [Year], [Month]);
GO
...
BEGIN
    ...
    SELECT TOP (1)
        ...
        pl.EmployeeDeductionAccountCode,
        pl.CompanyCostAccountCode,
        mi.StartDate,
        mi.EndDate
    ...
    WHERE mi.EmpId = @EmpId
      ...
      AND ISNULL(pl.IsActive, 1) = 1
      AND ISNULL(pl.LifecycleStatus, N'Active') NOT IN (N'Cancelled', N'Expired', N'Suspended')
