(function () {
    ...
    function setAdvanceWorkflow(data) {
        ...
        if (badge) {
            ...
            badge.textContent = data.StatusText || (data.Id ? "مسودة" : "طلب جديد");
        }
        ...
        list.innerHTML = "";
        [
            { label: "إدخال الطلب", done: !!data.Id, note: data.AdvanceDate || "" },
            { label: "إرسال للاعتماد", done: submitted, note: submitted ? "تم إرسال الطلب للمراجعة" : "لم يرسل بعد" },
            { label: rejected ? "مرفوض / ملغى" : "الاعتماد", done: approved || rejected, danger: rejected, note: data.StatusText || "" },
            { label: "الصرف", done: disbursed, note: disbursed ? ("سلفة فعلية رقم " + text(data.ActualAdvanceId || "")) : "لم تصرف بعد" },
            { label: "خصم الأقساط", done: hasPaid, note: hasPaid ? (text(data.PaidPartsCount) + " قسط مسدد/مرحل") : "لا توجد أقساط مسددة" }
        ...
        if (history.length) {
            ...
            historyItem.innerHTML = "<strong>سجل الاعتماد</strong><small>" + history.map(function (row) {
                return text((row.EmployeeName || row.FromUser || "مستخدم") + (row.ApprovedAt ? " - " + row.ApprovedAt : row.CancelledAt ? " - " + row.CancelledAt : ""));
            }).join(" | ") + "</small>";
    ...
    function showChangedComponent(data) {
        ...
        if (editor) {
            editor.hidden = false;
            if (editor.classList && editor.classList.contains("lhf-quick-entry")) {
    ...
    function updateChangedEntryPreview() {
        ...
        var periodWarning = parsedDate && year > 0 && month > 0 && (parsedDate.getFullYear() !== year || parsedDate.getMonth() + 1 !== month)
            ? "<em>تنبيه: تاريخ التسجيل خارج شهر/سنة المسير.</em>"
            : "";
    ...
    document.addEventListener("click", function (event) {
        ...
        if (saveVacationReturn) {
            ...
            var actualReturnDate = prompt("تاريخ المباشرة الفعلي بصيغة yyyy-mm-dd", saveVacationReturn.getAttribute("data-vacation-actual") || endDate);
            if (actualReturnDate === null) { return; }
            var actualDays = prompt("عدد أيام الإجازة الفعلية", saveVacationReturn.getAttribute("data-vacation-actual-days") || "");
            if (actualDays === null) { return; }
            var delayDays = prompt("عدد أيام التأخير", saveVacationReturn.getAttribute("data-vacation-delay-days") || "0");
        ...
        if (deleteVacation) {
            var confirmDeleteVacation = prompt("لحذف طلب الإجازة غير المعتمد نهائياً اكتب حذف. إذا دخل الطلب في اعتماد أو تسوية استخدم الإلغاء الآمن.");
            if (confirmDeleteVacation !== "حذف") { return; }
            if (!setButtonBusy(deleteVacation, true, "حذف...")) { return; }
            postForm(root.getAttribute("data-delete-vacation-url"), {
                id: deleteVacation.getAttribute("data-delete-vacation"),
                __RequestVerificationToken: token()
            }).then(function (res) {
                if (!success(res)) {
                    alert(message(res, "تعذر حذف طلب الإجازة."));
                    setButtonBusy(deleteVacation, false);
                    return;
                }
                alert(message(res, "تم حذف طلب الإجازة."));
                window.location.reload();
            }).catch(function () {
                alert("تعذر الاتصال بالخادم.");
                setButtonBusy(deleteVacation, false);
    ...
    if (changedForm) {
        if (!val("lhfChangedRecordDate")) {
            setCurrentPeriod("lhfChanged");
            updateChangedEntryPreview();
        }
