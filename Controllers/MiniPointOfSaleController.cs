...
{
    public class MiniPointOfSaleController : ViewToStringController
    {
        ...
        [SkipERPAuthorize]
        [AllowAnonymous]
        public ActionResult MiniPosLogin()
        {
            ...
            {
                Id = b.Id,
                ArName =  b.Code + " - " + b.EnName + " / " + b.ArName
            }), "Id", "ArName");
            return View();
        }
        [SkipERPAuthorize]
        [HttpPost]
        [AllowAnonymous]
        public ActionResult MiniPosLogin(string UserName, string Password, int ShiftId)
        {
            try
            {
                ...
                if (user == null)
                    return Content("UserIsNull");

                var User_UserName = user.UserName;
                ClaimsIdentity id = new ClaimsIdentity(new Claim[] { new Claim(ClaimTypes.Name, User_UserName), new Claim("Id", user.Id.ToString()), new Claim("RoleId", user.RoleId.ToString()) }, DefaultAuthenticationTypes.ApplicationCookie);
                ...
                db.ERPUser_GetHashPw(User_UserName, HashPW);
                string strHashPw = HashPW.Value.ToString();
                bool authenticated = PasswordEncrypt.VerifyHashPwd(Password, strHashPw);
                if (!authenticated)
                {
                    return Content("PasswordIncorrect");
                }
                else
                {
                    return Json(new { success = "success", ShiftId = ShiftId });
                }
            }
            catch (Exception ex)
            {
        ...
        //[SkipERPAuthorize]
        public async Task<ActionResult> AddEdit(int? id, int? selectedTableId, string selectedTableName, int? depId, int? selectedCustomerType, bool? showPaymentMethods,int?ShiftId)
        {
            var session = Session["lang"];
            ViewBag.ShiftId = ShiftId;
            ViewBag.SystemPageId = db.SystemPages.Where(a => a.IsActive == true && a.IsDeleted == false && a.ControllerName == "MiniPointOfSale").FirstOrDefault().Id;
            ViewBag.PaymentMethodId = null;
            SystemSetting systemSetting = await db.SystemSettings.AnyAsync() ? await db.SystemSettings.FirstOrDefaultAsync() : new SystemSetting();
            ...
            ViewBag.PaymentMethodId = salesInvoice.SalesInvoicePaymentMethods.FirstOrDefault().PaymentMethodId;
