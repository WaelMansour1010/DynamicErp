        ...
        public JsonResult GetPropertyContractTerminationDetails(string searchText, DateTime? terminationDate = null, bool useContractEndDate = false, int excludeTerminationId = 0)
        {
            ...
            var details = contracts.Select(a =>
            {
                ...
                var tempDetailsList = relevantBatches.Select(b =>
                {
                    ...
                    // ✅ حساب المجموع بدون السعي (Commission)
                    decimal batchTotalWithoutCommission = (b.BatchRentValue ?? 0) + (b.BatchRentValueTaxes ?? 0)
                    ...
                    // لا نضيف: BatchCommissionValue + BatchCommissionValueTaxes
                    ...
                    return new
                    {
                        ...
                        BatchTotalWithoutCommission = batchTotalWithoutCommission,  // ✅ المجموع بدون السعي
                ...
                if (lastFullyPaidBatch != null && lastFullyPaidBatch.BatchDate != DateTime.MinValue)
                {
                    ...
                    // ✅ متغير لحفظ فترة الدفعة بالأيام
                    int batchPeriodDays = 30; // قيمة افتراضية
                    ...
                    if (nextBatchAfterPaid != null && nextBatchAfterPaid.BatchDate.HasValue)
                    {
                        ...
                        // ✅ الإصلاح: حساب فترة الدفعة الفعلية
                        batchPeriodDays = (lastPaidBatchEndDate.Value.Date - lastPaidBatchStartDate.Date).Days;
                    }
                    else
                    {
                        // لو مفيش دفعة بعدها، نحاول نحسب الفترة من الدفعات السابقة
                        var orderedBatches = allBatches
                            .Where(b => b.BatchDate.HasValue)
                            .OrderBy(b => b.BatchDate)
                            .ToList();

                        if (orderedBatches.Count >= 2)
                        {
                            // نحسب الفترة من الفرق بين أول دفعتين
                            var firstBatchDate = orderedBatches[0].BatchDate.Value;
                            var secondBatchDate = orderedBatches[1].BatchDate.Value;
                            batchPeriodDays = (secondBatchDate.Date - firstBatchDate.Date).Days;

                            // نضيف الفترة لآخر دفعة مسددة
                            lastPaidBatchEndDate = lastPaidBatchStartDate.AddDays(batchPeriodDays);
                        }
                        else
                        {
                            // لو دفعة واحدة بس، نستخدم تاريخ نهاية العقد
                    ...
                    // ✅ التأكد إن فترة الدفعة منطقية
                    if (batchPeriodDays <= 0) batchPeriodDays = 30;
                    if (batchPeriodDays > 366) batchPeriodDays = 365; // سنة كحد أقصى

                    // 4️⃣ ✅ الإصلاح: حساب سعر اليوم بناءً على فترة الدفعة الفعلية (بدون السعي)
                    if (lastFullyPaidBatch.BatchTotalWithoutCommission > 0)
                    {
                        dailyRate = lastFullyPaidBatch.BatchTotalWithoutCommission / (decimal)batchPeriodDays;
                    }
                    else if (lastFullyPaidBatch.BatchTotal > 0)
                    {
                        // fallback: لو BatchTotalWithoutCommission = 0، نستخدم BatchTotal
                        dailyRate = lastFullyPaidBatch.BatchTotal / (decimal)batchPeriodDays;
                    }
                ...
                else
                {
                    ...
                    {
                        var firstBatch = orderedBatches[0];
                        var firstBatchStartDate = firstBatch.BatchDate.Value;

                        DateTime firstBatchEndDate;
                        int batchPeriodDays = 30;

                        if (orderedBatches.Count >= 2)
                        {
                            // نحسب الفترة من الفرق بين أول دفعتين
                            firstBatchEndDate = orderedBatches[1].BatchDate.Value;
                            batchPeriodDays = (firstBatchEndDate.Date - firstBatchStartDate.Date).Days;
                        }
                        ...
                        {
                            batchPeriodDays = 90;
                            firstBatchEndDate = firstBatchStartDate.AddDays(batchPeriodDays);
                        }

                        // التأكد إن الفترة منطقية
                        if (batchPeriodDays <= 0) batchPeriodDays = 30;

                        // ✅ حساب سعر اليوم بدون السعي (Commission)
                        decimal firstBatchTotalWithoutCommission = (firstBatch.BatchRentValue ?? 0) + (firstBatch.BatchRentValueTaxes ?? 0)
                                                                  + (firstBatch.BatchWaterValue ?? 0) + (firstBatch.BatchWaterValueTaxes ?? 0)
                                                                  + (firstBatch.BatchElectricityValue ?? 0) + (firstBatch.BatchElectricityValueTaxes ?? 0)
                                                                  + (firstBatch.BatchServicesValue ?? 0) + (firstBatch.BatchServicesValueTaxes ?? 0)
                                                                  + (firstBatch.BatchInsuranceValue ?? 0) + (firstBatch.BatchInsuranceValueTaxes ?? 0);

                        if (firstBatchTotalWithoutCommission > 0)
                        {
                            dailyRate = firstBatchTotalWithoutCommission / (decimal)batchPeriodDays;
                        }
                        else if (firstBatch.BatchTotal.HasValue && firstBatch.BatchTotal.Value > 0)
                        {
                            // fallback: لو مفيش قيم منفصلة، نستخدم BatchTotal
                            dailyRate = firstBatch.BatchTotal.Value / (decimal)batchPeriodDays;
                        }

                        // الفرق من نهاية أول دفعة
