...
BEGIN
    ...
    BEGIN TRY
        ...
        LEFT JOIN dbo.PropertyPilotCrossReference x
              ...
              AND x.OldId COLLATE DATABASE_DEFAULT = ra.Account_Code COLLATE DATABASE_DEFAULT
        ...
        LEFT JOIN dbo.PropertyPilotAccountMapping m
              ...
              AND m.OldAccountCode COLLATE DATABASE_DEFAULT = ra.Account_Code COLLATE DATABASE_DEFAULT
        WHERE m.Id IS NULL;
...
BEGIN
    ...
    BEGIN TRY
        ...
        ActiveContracts AS (
            ...
            SELECT DISTINCT r.CusID, r.CusName, r.CusNamee, r.RecordDate, r.NationalNo, r.ResponsibleContact, r.VATNO, r.Cus_Phone, r.Cus_mobile, r.Address, r.Account_Code FROM ActiveContracts c INNER JOIN Adnan.dbo.TblCustemers r ON r.CusID=c.CusID
        )
        ...
        FROM Tenants t
        LEFT JOIN dbo.PropertyPilotAccountMapping m
               ON m.MigrationBatchId=@MigrationBatchId AND m.OldDatabaseName=N'Adnan' AND m.OldAccountCode COLLATE DATABASE_DEFAULT=t.Account_Code COLLATE DATABASE_DEFAULT AND m.IsApproved=1
        LEFT JOIN dbo.PropertyPilotCrossReference x
...
BEGIN
    ...
    BEGIN TRY
        ...
        ActiveContracts AS (
            ...
            SELECT DISTINCT p.Aqarid, p.aqarNo, p.aqarname, p.aqartypeid, p.CountryID, p.cityid, p.heyid, p.streetname, p.floorcount, p.EntryCount, p.noofapartement, p.noofoffices, p.noofparking FROM ActiveContracts c INNER JOIN Adnan.dbo.TblAqar p ON p.Aqarid=c.Iqar
        )
...
BEGIN
    ...
    BEGIN TRY
        ...
        ActiveContracts AS (
            ...
            SELECT DISTINCT u.Id, u.Aqarid, u.unitno, u.unittype, u.Floor, u.roomscount, u.LoungeCount, u.haveFurniture, u.rentType, u.length, u.meterPrice, u.RentValue, u.MiniRentValue, u.kithchencount FROM ActiveContracts c INNER JOIN Adnan.dbo.TblAqarDetai u ON u.Id=c.UnitNo
        )
