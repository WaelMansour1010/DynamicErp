/* Phase6_EntityMigration_FIXED_SANDBOX_ONLY_20260520.sql - includes PropertyType/PropertyUnitType lookup mapping */
DECLARE @MigrationBatchId UNIQUEIDENTIFIER;
DECLARE @CutoverDate DATETIME;
SET @MigrationBatchId = 'D6EAD000-0000-4000-9000-202605200006';
...
BEGIN TRY
    ...
    SELECT @MigrationBatchId,N'Adnan',N'TblCustemers',CAST(t.CusID AS NVARCHAR(100)),N'PropertyRenter',pr.Id,N'Tenant',N'Phase6 fixed tenant migration'
    FROM #Tenants t
    ...
    SELECT N'ADNAN-P-' + CAST(p.Aqarid AS NVARCHAR(30)),
           ...
           p.aqarNo,ptm.NewId,p.CountryID,p.cityid,CAST(p.heyid AS NVARCHAR(100)),p.streetname,p.floorcount,p.EntryCount,p.noofapartement,p.noofoffices,p.noofparking,NULL
    FROM #Props p
    LEFT JOIN dbo.PropertyPilotLookupMapping ptm ON ptm.MappingGroup=N'PropertyType' AND ptm.OldId=CAST(p.aqartypeid AS NVARCHAR(100))
    ...
    INSERT INTO dbo.PropertyPilotCrossReference(MigrationBatchId,OldDatabaseName,OldTableName,OldId,NewTableName,NewId,EntityType,Notes)
    SELECT @MigrationBatchId,N'Adnan',N'TblAqar',CAST(p.Aqarid AS NVARCHAR(100)),N'Property',np.Id,N'Property',N'Phase6 fixed property migration'
    FROM #Props p
    ...
    SELECT px.NewId, CAST(u.unitno AS NVARCHAR(100)), utm.NewId, u.Floor, u.roomscount, u.LoungeCount,
    ...
    FROM #Units u
    LEFT JOIN dbo.PropertyPilotLookupMapping utm ON utm.MappingGroup=N'PropertyUnitType' AND utm.OldId=CAST(u.unittype AS NVARCHAR(100))
    ...
    INSERT INTO dbo.PropertyPilotCrossReference(MigrationBatchId,OldDatabaseName,OldTableName,OldId,NewTableName,NewId,EntityType,Notes)
    SELECT @MigrationBatchId,N'Adnan',N'TblAqarDetai',CAST(u.Id AS NVARCHAR(100)),N'PropertyDetail',pd.Id,N'Unit',N'Phase6 fixed unit migration'
    FROM #Units u
    ...
    SELECT N'ADNAN-C-' + CAST(CAST(c.ContNo AS int) AS NVARCHAR(30)),
           ...
           N'PropertyPilot Adnan ContNo=' + CAST(CAST(c.ContNo AS int) AS NVARCHAR(30)), ctm.NewId, ux.NewId,
    ...
    FROM #ActiveContracts c
    LEFT JOIN #Units cu ON cu.Id=c.UnitNo
    LEFT JOIN dbo.PropertyPilotLookupMapping ctm ON ctm.MappingGroup=N'PropertyUnitType' AND ctm.OldId=CAST(ISNULL(c.UnitType, cu.unittype) AS NVARCHAR(100))
    ...
    INSERT INTO dbo.PropertyPilotCrossReference(MigrationBatchId,OldDatabaseName,OldTableName,OldId,NewTableName,NewId,EntityType,Notes)
    SELECT @MigrationBatchId,N'Adnan',N'TblContract',CAST(CAST(c.ContNo AS int) AS NVARCHAR(100)),N'PropertyContract',pc.Id,N'Contract',N'Phase6 fixed contract migration'
    FROM #ActiveContracts c
    ...
    INSERT INTO dbo.PropertyPilotCrossReference(MigrationBatchId,OldDatabaseName,OldTableName,OldId,NewTableName,NewId,EntityType,Notes)
    SELECT @MigrationBatchId,N'Adnan',N'TblContractInstallments',CAST(i.id AS NVARCHAR(100)),N'PropertyContractBatch',pcb.Id,N'ContractBatch',N'Phase6 fixed batch migration'
    FROM Adnan.dbo.TblContractInstallments i
