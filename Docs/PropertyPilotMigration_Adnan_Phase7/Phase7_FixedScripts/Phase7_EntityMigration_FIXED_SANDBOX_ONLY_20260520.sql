/* Phase7_EntityMigration_FIXED_SANDBOX_ONLY_20260520.sql - includes PropertyType/PropertyUnitType lookup mapping */
...
BEGIN TRY
    ...
    SELECT @MigrationBatchId,N'Adnan',N'TblCustemers',CAST(t.CusID AS NVARCHAR(100)),N'PropertyRenter',pr.Id,N'Tenant',N'Phase7 fixed tenant migration'
    FROM #Tenants t
    ...
    INSERT INTO dbo.PropertyPilotCrossReference(MigrationBatchId,OldDatabaseName,OldTableName,OldId,NewTableName,NewId,EntityType,Notes)
    SELECT @MigrationBatchId,N'Adnan',N'TblAqar',CAST(p.Aqarid AS NVARCHAR(100)),N'Property',np.Id,N'Property',N'Phase7 fixed property migration'
    FROM #Props p
    ...
    INSERT INTO dbo.PropertyPilotCrossReference(MigrationBatchId,OldDatabaseName,OldTableName,OldId,NewTableName,NewId,EntityType,Notes)
    SELECT @MigrationBatchId,N'Adnan',N'TblAqarDetai',CAST(u.Id AS NVARCHAR(100)),N'PropertyDetail',pd.Id,N'Unit',N'Phase7 fixed unit migration'
    FROM #Units u
    ...
    INSERT INTO dbo.PropertyPilotCrossReference(MigrationBatchId,OldDatabaseName,OldTableName,OldId,NewTableName,NewId,EntityType,Notes)
    SELECT @MigrationBatchId,N'Adnan',N'TblContract',CAST(CAST(c.ContNo AS int) AS NVARCHAR(100)),N'PropertyContract',pc.Id,N'Contract',N'Phase7 fixed contract migration'
    FROM #ActiveContracts c
    ...
    INSERT INTO dbo.PropertyPilotCrossReference(MigrationBatchId,OldDatabaseName,OldTableName,OldId,NewTableName,NewId,EntityType,Notes)
    SELECT @MigrationBatchId,N'Adnan',N'TblContractInstallments',CAST(i.id AS NVARCHAR(100)),N'PropertyContractBatch',pcb.Id,N'ContractBatch',N'Phase7 fixed batch migration'
    FROM Adnan.dbo.TblContractInstallments i
