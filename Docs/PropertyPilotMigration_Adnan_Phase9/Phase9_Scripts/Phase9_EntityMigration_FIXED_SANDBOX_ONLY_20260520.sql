/* Phase9_EntityMigration_FIXED_SANDBOX_ONLY_20260520.sql - includes PropertyType/PropertyUnitType lookup mapping */
DECLARE @MigrationBatchId UNIQUEIDENTIFIER;
DECLARE @CutoverDate DATETIME;
SET @MigrationBatchId = 'F9EAD000-0000-4000-9000-202605200009';
...
BEGIN TRY
    ...
    SELECT @MigrationBatchId,N'Adnan',N'TblCustemers',CAST(t.CusID AS NVARCHAR(100)),N'PropertyRenter',pr.Id,N'Tenant',N'Phase9 fixed tenant migration'
    FROM #Tenants t
    ...
    INSERT INTO dbo.PropertyPilotCrossReference(MigrationBatchId,OldDatabaseName,OldTableName,OldId,NewTableName,NewId,EntityType,Notes)
    SELECT @MigrationBatchId,N'Adnan',N'TblAqar',CAST(p.Aqarid AS NVARCHAR(100)),N'Property',np.Id,N'Property',N'Phase9 fixed property migration'
    FROM #Props p
    ...
    INSERT INTO dbo.PropertyPilotCrossReference(MigrationBatchId,OldDatabaseName,OldTableName,OldId,NewTableName,NewId,EntityType,Notes)
    SELECT @MigrationBatchId,N'Adnan',N'TblAqarDetai',CAST(u.Id AS NVARCHAR(100)),N'PropertyDetail',pd.Id,N'Unit',N'Phase9 fixed unit migration'
    FROM #Units u
    ...
    INSERT INTO dbo.PropertyPilotCrossReference(MigrationBatchId,OldDatabaseName,OldTableName,OldId,NewTableName,NewId,EntityType,Notes)
    SELECT @MigrationBatchId,N'Adnan',N'TblContract',CAST(CAST(c.ContNo AS int) AS NVARCHAR(100)),N'PropertyContract',pc.Id,N'Contract',N'Phase9 fixed contract migration'
    FROM #ActiveContracts c
    ...
    INSERT INTO dbo.PropertyPilotCrossReference(MigrationBatchId,OldDatabaseName,OldTableName,OldId,NewTableName,NewId,EntityType,Notes)
    SELECT @MigrationBatchId,N'Adnan',N'TblContractInstallments',CAST(i.id AS NVARCHAR(100)),N'PropertyContractBatch',pcb.Id,N'ContractBatch',N'Phase9 fixed batch migration'
    FROM Adnan.dbo.TblContractInstallments i
