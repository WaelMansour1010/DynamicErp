    ...
    public class CashReceiptVoucherController : Controller
    {
        ...
        public JsonResult GetSettlementDetails(int id)
        {
            try
            {
                ...
                // ✅ 2) تحديد تاريخ نهاية الحساب حسب الـ CalculationMethod
                // 0 = بتاريخ التصفية ، 1 = بنهاية العقد (أو آخر دفعة)
                bool useContractEndDate = (termination.CalculationMethod ?? 0) == 1;

                DateTime referenceDate = termination.TerminationDate ?? DateTime.Now;
                DateTime calculationEndDate;
                List<PropertyContractBatch> relevantBatches;

                if (useContractEndDate)
                {
                    // "بنهاية العقد" → نجيب كل الدفعات بالترتيب
                    var allBatches = contract.PropertyContractBatches
                        .Where(b => b.BatchDate.HasValue)
                        .OrderBy(b => b.BatchDate)
                        .ToList();

                    var lastBatchDate = allBatches.LastOrDefault()?.BatchDate;
                    calculationEndDate = lastBatchDate.HasValue
                        ? lastBatchDate.Value
                        : (contract.ContractEndDate ?? referenceDate);

                    relevantBatches = allBatches;
                }
                else
                {
                    // "بتاريخ التصفية" → نفلتر على الدفعات اللي تاريخها <= تاريخ التصفية
                    calculationEndDate = referenceDate;

                    relevantBatches = contract.PropertyContractBatches
                        .Where(b => b.BatchDate.HasValue && b.BatchDate.Value <= calculationEndDate)
                        .OrderBy(b => b.BatchDate)
                        .ToList();
                }

                // ✅ 3) جلب المدفوعات من جدول السداد CashReceiptVoucherPropertyContractBatch
                var batchIds = relevantBatches.Select(b => b.Id).ToList();
                // ✅ حساب إجمالي التلفيات من جدول PropertyContractTerminationDamage (Qty * Price)
                // ✅ حساب إجمالي التلفيات (Qty * Price) من جدول التلفيات
                var totalDamages = db.PropertyContractTerminationDamages
                    .Where(d => d.PropertyContractTerminationId == termination.Id)
                    .Select(d => ((decimal)(d.Qty ?? 0)) * (d.Price ?? 0))
                    .DefaultIfEmpty(0)
                    .Sum();

                // Fix for CS1061: 'IGrouping<int, CashReceiptVoucherPropertyContractBatch>' does not contain a definition for 'Qty' or 'Price'.
                // The issue arises because the `CashReceiptVoucherPropertyContractBatch` class does not have `Qty` or `Price` properties.
                // Instead, you should use the properties that exist in the `CashReceiptVoucherPropertyContractBatch` class or its related entities.
                ...
                // ✅ 4) حساب تفاصيل الدفعات المتبقية بنفس منطق التصفية
                ...
                var result = new
                {
                    ...
                    // أيام النقص (لو حابب تستعملهم في الفيو برضه)
                    ...
                    // ✅ إجمالي التلفيات (متخزن في جدول التصفية بعد ما حسبناه في الـ SP)
                ...
                // ✅ التأمين من تفاصيل الدفعات (مش من فيلد مستقل)
                InsuranceAmount = insuranceAmount,

                    // ✅ رصيد المستأجر المجمع من التصفية
                    RenterBalance = renterBalance,

                    // ✅ إجمالي الدفعات غير المسددة (وفق منطق التصفية والسداد)
