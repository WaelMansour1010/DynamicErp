    ...
    public class PropertyContractTerminationController : Controller
    {
        ...
        public JsonResult GetPropertyContractTerminationDetails(string searchText, DateTime? terminationDate = null, bool useContractEndDate = false, int excludeTerminationId = 0)
        {
            ...
            var details = contracts.Select(a =>
            {
                ...
                // 1️⃣ نجيب تفاصيل الدفعات مع حالة الدفع (مؤقتاً)
                ...
                var lastFullyPaidBatch = tempDetailsList
                    ...
                    .OrderBy(d => d.Batch.BatchDate)
                    .LastOrDefault();

                if (lastFullyPaidBatch != null && lastFullyPaidBatch.Batch.BatchDate.HasValue)
                {
                    // 3️⃣ حساب تاريخ **نهاية** آخر دفعة مسددة
                    var lastPaidBatchStartDate = lastFullyPaidBatch.Batch.BatchDate.Value;

                    // نشوف هل في دفعة بعدها؟
                    var nextBatch = relevantBatches
                        .Where(b => b.BatchDate.HasValue && b.BatchDate.Value > lastPaidBatchStartDate)
                        .OrderBy(b => b.BatchDate)
                        .FirstOrDefault();

                    if (nextBatch != null && nextBatch.BatchDate.HasValue)
                    {
                        // تاريخ نهاية آخر دفعة مسددة = تاريخ بداية الدفعة التالية
                        lastPaidBatchEndDate = nextBatch.BatchDate.Value;
                    }
                    else
                    {
                        // لو مفيش دفعة بعدها، نستخدم تاريخ نهاية العقد أو نضيف شهر
                        ...
                        {
                            // نضيف شهر واحد على تاريخ بداية آخر دفعة مسددة
                            lastPaidBatchEndDate = lastPaidBatchStartDate.AddMonths(1);
                        }
                    }

                    // 4️⃣ حساب سعر اليوم من آخر دفعة مسددة
                    if (lastFullyPaidBatch.Batch.BatchTotal.HasValue && lastFullyPaidBatch.Batch.BatchTotal.Value > 0)
                    {
                        dailyRate = lastFullyPaidBatch.Batch.BatchTotal.Value / 30m;
                    }

                    // 5️⃣ حساب الفرق بين تاريخ التصفية و **نهاية** آخر دفعة مسددة
                ...
                {
                    // لو مفيش دفعات مسددة، نستخدم المنطق القديم
                    var firstBatch = relevantBatches.FirstOrDefault();
                    if (firstBatch != null && firstBatch.BatchDate.HasValue)
                    {
                        ...
                        // الفرق من بداية أول دفعة
                        int daysDiff = (calculationEndDate.Date - firstBatch.BatchDate.Value.Date).Days;
