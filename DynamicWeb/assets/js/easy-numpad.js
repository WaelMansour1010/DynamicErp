...
function easynum() {
    ...
    if (selectedPaymentMethodInput != null) {
        //console.log(selectedPaymentMethodInput.inputmask('unmaskedvalue') * 1)

        ////////old code
        //selectedPaymentMethodInput.val(selectedPaymentMethodInput.inputmask('unmaskedvalue') * 1 + easy_num_value);
        //selectedPaymentMethodInput.trigger("change");

        if (easy_num_value != '.') {
            if (tempVal != "" && tempInput != "" && tempInput == selectedPaymentMethodInput) {
                var newInputVal = tempVal + easy_num_value;
                selectedPaymentMethodInput.val(newInputVal);
                tempVal = "";
                tempInput = "";
            }
            else {
                var newInputVal = selectedPaymentMethodInput.inputmask('unmaskedvalue') * 1 + easy_num_value;
                selectedPaymentMethodInput.val(newInputVal);
            }
            ...
            tempVal = selectedPaymentMethodInput.inputmask('unmaskedvalue') * 1 + easy_num_value;
            tempInput = selectedPaymentMethodInput;
        }
    }
...
function easy_numpad_cancel() {
    event.preventDefault();
    //var easy_numpad_output_val = selectedPaymentMethodInput.inputmask('unmaskedvalue') * 1;
    //var easy_numpad_output_val_deleted = easy_numpad_output_val.slice(0, -1);
    //selectedPaymentMethodInput.val(easy_numpad_output_val_deleted);
...
function easy_numpad_del() {
    event.preventDefault();
    var oldVal = selectedPaymentMethodInput.inputmask('unmaskedvalue') * 1;
    var newVal = oldVal.toString().substring(0, oldVal.toString().length - 1);
    //console.log(newVal);
    selectedPaymentMethodInput.val(newVal);
    selectedPaymentMethodInput.trigger("change");
