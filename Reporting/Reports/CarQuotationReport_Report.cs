...
namespace MyERP.Reporting.Reports
{
    public class CarQuotationReport_Report : XtraReport
    {
        ...
        public CarQuotationReport_Report(DateTime? fromDate, DateTime? toDate, int? customerId, int? carTypeId, int? carModelId)
        {
            ...
            Parameters.AddRange(new[] { pFromDate, pToDate, pCustomerId, pCarTypeId, pCarModelId });

            var sqlDataSource = new SqlDataSource("ConnectionString");
            var query = new StoredProcQuery("CarQuotationReport_Get", "CarQuotationReport_Get");
          ...
          //  TopMargin.HeightF = 20;
            ...
            {
                Text = "تقرير عروض أسعار السيارات",
            ...
            foreach (var ex in new[] { "[QuotationNumber]", "[QuotationDate]", "[CustomerName]", "[ChassisNo]", "[CarType]", "[CarModel]", "[CarColor]", "[ManufacturingYear]", "[EngineNo]", "[PlateNo]", "[Price]", "[VehicleNotes]" })
                AddCell(row, ex);
            table.Rows.Add(row);
            ...
            foreach (var txt in new[] { "رقم العرض", "تاريخ العرض", "العميل", "الشاسيه", "النوع", "الموديل", "اللون", "سنة الصنع", "المحرك", "اللوحة", "السعر", "ملاحظات" })
