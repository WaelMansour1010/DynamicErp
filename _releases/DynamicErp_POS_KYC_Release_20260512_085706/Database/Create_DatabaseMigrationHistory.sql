...
BEGIN
    ...
    (
        RunId BIGINT IDENTITY(1,1) NOT NULL
            CONSTRAINT PK_DatabaseMigrationRun PRIMARY KEY,
        StartedAt DATETIME NOT NULL
            CONSTRAINT DF_DatabaseMigrationRun_StartedAt DEFAULT (GETDATE()),
        ...
        TotalScripts INT NOT NULL
            CONSTRAINT DF_DatabaseMigrationRun_TotalScripts DEFAULT (0),
        AppliedCount INT NOT NULL
            CONSTRAINT DF_DatabaseMigrationRun_AppliedCount DEFAULT (0),
        FailedCount INT NOT NULL
            CONSTRAINT DF_DatabaseMigrationRun_FailedCount DEFAULT (0),
        WarningCount INT NOT NULL
            CONSTRAINT DF_DatabaseMigrationRun_WarningCount DEFAULT (0)
    );
END;
GO
...
BEGIN
    ...
    (
        RunDetailId BIGINT IDENTITY(1,1) NOT NULL
            CONSTRAINT PK_DatabaseMigrationRunDetail PRIMARY KEY,
...
(
    SELECT 1
    FROM sys.indexes
    WHERE name = N'IX_DatabaseMigrationRunDetail_RunId'
      AND object_id = OBJECT_ID(N'dbo.DatabaseMigrationRunDetail', N'U')
)
BEGIN
