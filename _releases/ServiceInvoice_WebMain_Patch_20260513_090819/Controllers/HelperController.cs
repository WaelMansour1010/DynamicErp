...
namespace MyERP.Controllers
{
    public class HelperController : Controller
    {
        ...
        public static string EncodeTLV(string Seller, string TaxNumber, string InvoiceDate, string InvoiceTotalAmount, string InvoiceTaxAmount)
        {
            ...
            {
                WriteTlvField(stream, 1, Seller);
                WriteTlvField(stream, 2, TaxNumber);
                WriteTlvField(stream, 3, InvoiceDate);
                WriteTlvField(stream, 4, InvoiceTotalAmount);
                WriteTlvField(stream, 5, InvoiceTaxAmount);

                return Convert.ToBase64String(stream.ToArray());
            }
        }

        private static void WriteTlvField(Stream stream, byte tag, string value)
        {
            var bytes = Encoding.UTF8.GetBytes(value ?? string.Empty);
            if (bytes.Length > byte.MaxValue)
            {
                throw new ArgumentException("TLV field value is too long.", nameof(value));
            }
